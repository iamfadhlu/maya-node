...
type SlashingV104Suite struct{}

var _ = Suite(&SlashingV104Suite{})

type TestSlashingLackKeeperV104 struct {
	keeper.KVStoreDummy
	txOut                      *TxOut
	na                         NodeAccount
	vaults                     Vaults
...
func (k *TestSlashingLackKeeperV104) PoolExist(ctx cosmos.Context, asset common.Asset) bool {
	return k.pools[asset].IsEmpty()
}

func (k *TestSlashingLackKeeperV104) GetObservedTxInVoter(_ cosmos.Context, _ common.TxID) (ObservedTxVoter, error) {
...
func (k *TestSlashingLackKeeperV104) SetObservedTxInVoter(_ cosmos.Context, voter ObservedTxVoter) {
	k.voter = voter
}

func (k *TestSlashingLackKeeperV104) GetVault(_ cosmos.Context, pk common.PubKey) (Vault, error) {
	if k.failGetVault {
...
func (k *TestSlashingLackKeeperV104) GetAsgardVaultsByStatus(_ cosmos.Context, _ VaultStatus) (Vaults, error) {
...
func (k *TestSlashingLackKeeperV104) GetTxOut(_ cosmos.Context, _ int64) (*TxOut, error) {
	if k.failGetTxOut {
		return nil, errKaboom
	}
	return k.txOut, nil
...
func (k *TestSlashingLackKeeperV104) SetTxOut(_ cosmos.Context, tx *TxOut) error {
	if k.failSetTxOut {
		return errKaboom
	}
	k.txOut = tx
...
func (k *TestSlashingLackKeeperV104) IncNodeAccountSlashPoints(_ cosmos.Context, addr cosmos.AccAddress, pts int64) error {
...
func (k *TestSlashingLackKeeperV104) GetNodeAccountByPubKey(_ cosmos.Context, _ common.PubKey) (NodeAccount, error) {
...
func (k *TestSlashingLackKeeperV104) SetNodeAccount(_ cosmos.Context, na NodeAccount) error {
...
type TestSlashObservingKeeperV104 struct {
	keeper.KVStoreDummy
...
func (k *TestSlashObservingKeeperV104) GetObservingAddresses(_ cosmos.Context) ([]cosmos.AccAddress, error) {
...
func (k *TestSlashObservingKeeperV104) ClearObservingAddresses(_ cosmos.Context) {
	k.addrs = nil
}

func (k *TestSlashObservingKeeperV104) IncNodeAccountSlashPoints(_ cosmos.Context, addr cosmos.AccAddress, pts int64) error {
	if _, ok := k.slashPts[addr.String()]; !ok {
		k.slashPts[addr.String()] = 0
	}
	k.slashPts[addr.String()] += pts
...
func (k *TestSlashObservingKeeperV104) ListActiveValidators(_ cosmos.Context) (NodeAccounts, error) {
...
func (k *TestSlashObservingKeeperV104) SetNodeAccount(_ cosmos.Context, na NodeAccount) error {
	if k.failSetNodeAccount {
		return errKaboom
	}
	for i := range k.nas {
...
type TestDoubleSlashKeeperV104 struct {
	keeper.KVStoreDummy
	na          NodeAccount
	naBond      cosmos.Uint
	bp          BondProviders
...
func (s *SlashingV104Suite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (s *SlashingV104Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := newSlasherV104(k, NewDummyEventMgr())
...
func (s *SlashingV104Suite) TestLackObservingErrors(c *C) {
	...
	keeper := &TestSlashObservingKeeperV104{
		nas:      nas,
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV104(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV104Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
		condition   func(keeper *TestSlashingLackKeeperV104)
		shouldError bool
	}{
		{
	...
	for _, item := range testCases {
		...
		keeper := &TestSlashingLackKeeperV104{
			txOut:  txOut,
			na:     na,
			vaults: Vaults{ygg},
			voter: ObservedTxVoter{
...
func (s *SlashingV104Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV104Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomValidatorNode(NodeActive),
		GetRandomValidatorNode(NodeActive),
	}
...
func (s *SlashingV104Suite) TestDoubleSign(c *C) {
	...
	slasher := newSlasherV104(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV104Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
	...
	keeper := &TestDoubleSlashKeeperV104{
		na:     na,
		naBond: naBond,
		bp:     bp,
		lp: LiquidityProvider{
...
func (s *SlashingV104Suite) TestSlashVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV104(mgr.Keeper(), mgr.EventMgr())
...
type TestSlashNodeAccountLPKeeperV104 struct {
	keeper.Keeper
	failCalcBond                     bool
	zeroBond                         bool
	failGetPolAddr                   bool
...
func (k *TestSlashNodeAccountLPKeeperV104) CalcNodeLiquidityBond(ctx cosmos.Context, na NodeAccount) (cosmos.Uint, error) {
	if k.failCalcBond {
...
func (k *TestSlashNodeAccountLPKeeperV104) GetModuleAddress(module string) (common.Address, error) {
	if k.failGetPolAddr {
...
func (k *TestSlashNodeAccountLPKeeperV104) GetBondProviders(ctx cosmos.Context, nodeAddr cosmos.AccAddress) (BondProviders, error) {
...
func (k *TestSlashNodeAccountLPKeeperV104) GetLiquidityProviderByAssets(ctx cosmos.Context, assets common.Assets, assetAddr common.Address) (LiquidityProviders, error) {
...
func (k *TestSlashNodeAccountLPKeeperV104) GetPool(ctx cosmos.Context, asset common.Asset) (Pool, error) {
	if k.failGetPool {
...
func (k *TestSlashNodeAccountLPKeeperV104) GetLiquidityProvider(ctx cosmos.Context, asset common.Asset, lpAddr common.Address) (LiquidityProvider, error) {
	if k.failGetLP {
...
func (s *SlashingV104Suite) TestSlashNodeAccountLP(c *C) {
	ctx, mgr := setupManagerForTest(c)
	keeper := &TestSlashNodeAccountLPKeeperV104{
		Keeper: mgr.Keeper(),
	}

	slasher := newSlasherV104(keeper, mgr.EventMgr())
	// when slash is zero
...
func (s *SlashingV104Suite) TestNetworkShouldNotSlashMorethanVaultAmount(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV104(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV104Suite) TestNeedsNewVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
