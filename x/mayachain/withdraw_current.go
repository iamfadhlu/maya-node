...
// withdrawV124 handles withdrawal with reserve balance checks for ILP
// it returns runeAmt,assetAmount,protectionRuneAmt,units, lastWithdraw,err
func withdrawV124(ctx cosmos.Context, msg MsgWithdrawLiquidity, mgr Manager) (cosmos.Uint, cosmos.Uint, cosmos.Uint, cosmos.Uint, cosmos.Uint, error) {
	...
	if fullProtectionLine > 0 && pool.Status == PoolAvailable && !(ilpDisabled > 0 && !pool.Asset.IsVaultAsset()) { // if protection line is zero, no imp loss protection is given
		...
		// Check if Reserve has sufficient balance to cover ILP
		if !implProtectionCacaoAmount.IsZero() {
			reserveBalance := mgr.Keeper().GetRuneBalanceOfModule(ctx, ReserveName)
			if reserveBalance.LT(implProtectionCacaoAmount) {
				// Log the event but don't apply ILP if reserve is insufficient
				ctx.Logger().Error("insufficient reserve balance for ILP, proceeding without protection",
					"required", implProtectionCacaoAmount,
					"available", reserveBalance,
					"pool", pool.Asset,
					"lp", lp.CacaoAddress)
				// Continue withdrawal without ILP protection
				// protectionCacaoAmount remains zero as initialized
			} else {
