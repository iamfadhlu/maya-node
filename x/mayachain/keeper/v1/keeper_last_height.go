...
// ForceSetLastObserveHeight force set the last observe height, bypassing the height check
// This should only be used in migrations to fix incorrect heights
func (k KVStore) ForceSetLastObserveHeight(ctx cosmos.Context, chain common.Chain, address cosmos.AccAddress, height int64) error {
	key := k.GetKey(ctx, prefixLastObserveHeight, address.String()) + "/" + chain.String()
	k.setInt64(ctx, key, height)
	return nil
}

// ForceSetLastChainHeight force set the last chain height, bypassing the height check
// This should only be used in migrations to fix incorrect heights
func (k KVStore) ForceSetLastChainHeight(ctx cosmos.Context, chain common.Chain, height int64) error {
	k.setInt64(ctx, k.GetKey(ctx, prefixLastChainHeight, chain.String()), height)
	return nil
}
