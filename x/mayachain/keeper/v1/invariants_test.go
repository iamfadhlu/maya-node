...
func (s *InvariantsSuite) TestAffiliateCollectorInvariant(c *C) {
	ctx, k := setupKeeperForTest(c)

	invariant := AffiliateCollectorInvariant(k)
	...
	// send some coins to make it oversolvent
	coins := common.NewCoins(common.NewCoin(common.BaseAsset(), cosmos.NewUint(1)))
	c.Assert(k.SendFromModuleToModule(ctx, AsgardName, AffiliateCollectorName, coins), IsNil)
	...
	// create MAYAName and add the same amount of coins to it's affiliate collector
	addr := GetRandomBaseAddress()
	acc, _ := addr.AccAddress()
	name := NewMAYAName("hello", 50, []MAYANameAlias{{Chain: common.BASEChain, Address: GetRandomBaseAddress()}}, common.EmptyAsset, acc, cosmos.ZeroUint(), nil)
	k.SetMAYAName(ctx, name)
	ac, err := k.GetAffiliateCollector(ctx, acc)
	c.Assert(err, IsNil)
	ac.CacaoAmount = coins[0].Amount
	k.SetAffiliateCollector(ctx, ac)

	// should pass since it has same amount of coins
	msg, broken = invariant(ctx)
	c.Assert(broken, Equals, false)
	c.Assert(msg, IsNil)
}
