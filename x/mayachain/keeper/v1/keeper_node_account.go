...
// ListValidatorsByStatus - get a list of validator node accounts with the given statuses (bitwise OR)
func (k KVStore) ListValidatorsByStatuses(ctx cosmos.Context, statuses ...NodeStatus) (NodeAccounts, error) {
	nodeAccounts := make(NodeAccounts, 0)
	naIterator := k.GetNodeAccountIterator(ctx)
	defer naIterator.Close()
	for ; naIterator.Valid(); naIterator.Next() {
...
// CalcNodeLiquidityBond takes the liquidity provided by every bp of a NodeAccount and calculates its cacao side value based on LP Units
...
// CalcNodeBondProvidersLiquidityBond takes the liquidity provided by every bp of a NodeAccount and calculates its cacao side value based on LP Units and returns the total bond and a slice of the bond for each provider
func (k KVStore) CalcNodeBondProvidersLiquidityBond(ctx cosmos.Context, na NodeAccount) (cosmos.Uint, []cosmos.Uint, error) {
	nodeBond := cosmos.ZeroUint()
	bp, err := k.GetBondProviders(ctx, na.NodeAddress)
	if err != nil {
		return nodeBond, nil, err
	}

	bpBonds := make([]cosmos.Uint, len(bp.Providers))
	for i, b := range bp.Providers {
		bpBonds[i] = cosmos.ZeroUint()
		if !b.HasBonded() {
			continue
		}
