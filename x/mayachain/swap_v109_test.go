...
type SwapV109Suite struct{}

var _ = Suite(&SwapV109Suite{})

func (s *SwapV109Suite) SetUpSuite(c *C) {
...
func (s *SwapV109Suite) TestSwap(c *C) {
	...
	for _, item := range inputs {
		...
		amount, evts, err := newSwapperV109().Swap(ctx, poolStorage, tx, item.target, item.destination, item.tradeTarget, "", "", nil, StreamingSwap{}, cosmos.NewUint(1000_000), 20_000, mgr)
...
func (s *SwapV109Suite) TestSynthSwap_RuneSynthRune(c *C) {
	ctx, mgr := setupManagerForTest(c)
	pool := NewPool()
	...
	// swap rune --> synth
	{
		...
		amount, _, err := newSwapperV109().Swap(ctx, mgr.Keeper(), tx, common.BNBAsset.GetSyntheticAsset(), addr, cosmos.ZeroUint(), "", "", nil, StreamingSwap{}, cosmos.NewUint(1000_000), 20_000, mgr)
		c.Assert(err, IsNil)
	...
	// swap synth --> rune
	{
		...
		amount, _, err := newSwapperV109().Swap(ctx, mgr.Keeper(), tx, common.BaseAsset(), addr, cosmos.ZeroUint(), "", "", nil, StreamingSwap{}, cosmos.NewUint(1000_000), 20_000, mgr)
		c.Assert(err, IsNil)
		c.Check(amount.Uint64(), Equals, swapResult.Uint64(),
			Commentf("Actual: %d Exp: %d", amount.Uint64(), swapResult.Uint64()))
		pool, err = mgr.Keeper().GetPool(ctx, common.BNBAsset)
...
func (s *SwapV109Suite) TestSynthSwap_AssetSynth(c *C) {
	ctx, mgr := setupManagerForTest(c)
	pool := NewPool()
	pool.Asset = common.BNBAsset
	...
	amount, _, err = newSwapperV109().Swap(ctx, mgr.Keeper(), tx1, common.BTCAsset, addr, cosmos.ZeroUint(), "", "", nil, StreamingSwap{}, cosmos.NewUint(1000_000_000_000), 20_000, mgr)
	c.Assert(err, NotNil)
