...
func (ob *OrderBookVCUR) FetchQueue(ctx cosmos.Context, mgr Manager, pairs tradePairs, pools Pools) (orderItems, error) { // nolint
	...
	// be checked (proc).
	poolCycle := ob.k.GetConfigInt64(ctx, constants.PoolCycle)
...
func (ob *OrderBookVCUR) EndBlock(ctx cosmos.Context, mgr Manager) error {
	...
	swaps, err = ob.scoreMsgs(ctx, swaps, synthVirtualDepthMult, mgr)
	if err != nil {
	...
	for i := int64(0); i < ob.getTodoNum(int64(len(swaps)), minSwapsPerBlock, maxSwapsPerBlock); i++ {
		...
		if !msg.AffiliateBasisPoints.IsZero() && msg.AffiliateAddress.IsChain(common.THORChain, mgr.GetVersion()) {
...
// orderItem list
func (ob *OrderBookVCUR) scoreMsgs(ctx cosmos.Context, items orderItems, synthVirtualDepthMult int64, mgr Manager) (orderItems, error) {
	...
	for i, item := range items {
		...
		ob.getLiquidityFeeAndSlip(ctx, pool, item.msg.Tx.Coins[0], &items[i], virtualDepthMult, mgr)
		...
		ob.getLiquidityFeeAndSlip(ctx, pool, runeCoin, &items[i], virtualDepthMult, mgr)
	}
...
func (ob *OrderBookVCUR) getLiquidityFeeAndSlip(ctx cosmos.Context, pool Pool, sourceCoin common.Coin, item *orderItem, virtualDepthMult int64, mgr Manager) {
