...
type SwapperV112 struct{}

func newSwapperV112() *SwapperV112 {
	return &SwapperV112{}
}

// validateMessage is trying to validate the legitimacy of the incoming message and decide whether THORNode can handle it
func (s *SwapperV112) validateMessage(tx common.Tx, target common.Asset, destination common.Address) error {
...
func (s *SwapperV112) Swap(ctx cosmos.Context,
...
func (s *SwapperV112) swapOne(ctx cosmos.Context,
...
// nolint
func (s *SwapperV112) CalcAssetEmission(X, x, Y cosmos.Uint) cosmos.Uint {
	// ( x * X * Y ) / ( x + X )^2
...
// nolint
func (s *SwapperV112) CalcLiquidityFee(X, x, Y cosmos.Uint) cosmos.Uint {
	// ( x^2 *  Y ) / ( x + X )^2
	numerator := x.Mul(x).Mul(Y)
	denominator := x.Add(X).Mul(x.Add(X))
	if denominator.IsZero() {
...
// nolint
func (s *SwapperV112) CalcSwapSlip(Xi, xi cosmos.Uint, slipFeeAddedBasisPoints cosmos.Uint) cosmos.Uint {
	// Cast to DECs
