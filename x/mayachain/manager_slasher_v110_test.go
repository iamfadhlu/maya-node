...
type SlashingV110Suite struct{}

var _ = Suite(&SlashingV110Suite{})

type TestSlashingLackKeeperV110 struct {
	keeper.KVStoreDummy
	txOut                      *TxOut
	na                         NodeAccount
	vaults                     Vaults
...
func (k *TestSlashingLackKeeperV110) PoolExist(ctx cosmos.Context, asset common.Asset) bool {
	return k.pools[asset].IsEmpty()
}

func (k *TestSlashingLackKeeperV110) GetObservedTxInVoter(_ cosmos.Context, _ common.TxID) (ObservedTxVoter, error) {
...
func (k *TestSlashingLackKeeperV110) SetObservedTxInVoter(_ cosmos.Context, voter ObservedTxVoter) {
	k.voter = voter
}

func (k *TestSlashingLackKeeperV110) GetVault(_ cosmos.Context, pk common.PubKey) (Vault, error) {
	if k.failGetVault {
...
func (k *TestSlashingLackKeeperV110) GetAsgardVaultsByStatus(_ cosmos.Context, _ VaultStatus) (Vaults, error) {
...
func (k *TestSlashingLackKeeperV110) GetTxOut(_ cosmos.Context, _ int64) (*TxOut, error) {
	if k.failGetTxOut {
		return nil, errKaboom
	}
	return k.txOut, nil
...
func (k *TestSlashingLackKeeperV110) SetTxOut(_ cosmos.Context, tx *TxOut) error {
	if k.failSetTxOut {
		return errKaboom
	}
	k.txOut = tx
...
func (k *TestSlashingLackKeeperV110) IncNodeAccountSlashPoints(_ cosmos.Context, addr cosmos.AccAddress, pts int64) error {
...
func (k *TestSlashingLackKeeperV110) GetNodeAccountByPubKey(_ cosmos.Context, _ common.PubKey) (NodeAccount, error) {
...
func (k *TestSlashingLackKeeperV110) SetNodeAccount(_ cosmos.Context, na NodeAccount) error {
...
type TestSlashObservingKeeperV110 struct {
	keeper.KVStoreDummy
...
func (k *TestSlashObservingKeeperV110) GetObservingAddresses(_ cosmos.Context) ([]cosmos.AccAddress, error) {
...
func (k *TestSlashObservingKeeperV110) ClearObservingAddresses(_ cosmos.Context) {
	k.addrs = nil
}

func (k *TestSlashObservingKeeperV110) IncNodeAccountSlashPoints(_ cosmos.Context, addr cosmos.AccAddress, pts int64) error {
	if _, ok := k.slashPts[addr.String()]; !ok {
		k.slashPts[addr.String()] = 0
	}
	k.slashPts[addr.String()] += pts
...
func (k *TestSlashObservingKeeperV110) ListActiveValidators(_ cosmos.Context) (NodeAccounts, error) {
...
func (k *TestSlashObservingKeeperV110) SetNodeAccount(_ cosmos.Context, na NodeAccount) error {
	if k.failSetNodeAccount {
		return errKaboom
	}
	for i := range k.nas {
...
type TestDoubleSlashKeeperV110 struct {
	keeper.KVStoreDummy
	na          NodeAccount
	naBond      cosmos.Uint
	bp          BondProviders
...
func (s *SlashingV110Suite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (s *SlashingV110Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := newSlasherV110(k, NewDummyEventMgr())
...
func (s *SlashingV110Suite) TestLackObservingErrors(c *C) {
	...
	keeper := &TestSlashObservingKeeperV110{
		nas:      nas,
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV110(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV110Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
		condition   func(keeper *TestSlashingLackKeeperV110)
		shouldError bool
	}{
		{
	...
	for _, item := range testCases {
		...
		keeper := &TestSlashingLackKeeperV110{
			txOut:  txOut,
			na:     na,
			vaults: Vaults{ygg},
			voter: ObservedTxVoter{
...
func (s *SlashingV110Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV110Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomValidatorNode(NodeActive),
		GetRandomValidatorNode(NodeActive),
	}
...
func (s *SlashingV110Suite) TestDoubleSign(c *C) {
	...
	SetupLiquidityBondForTestV105(c, ctx, mgr.Keeper(), common.BTCAsset, na.BondAddress, na, naBond)
	...
	slasher := newSlasherV110(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV110Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
	...
	keeper := &TestDoubleSlashKeeperV110{
		na:     na,
		naBond: naBond,
		bp:     bp,
		lp: LiquidityProvider{
...
func (s *SlashingV110Suite) TestSlashVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV110(mgr.Keeper(), mgr.EventMgr())
	...
	SetupLiquidityBondForTestV105(c, ctx, mgr.Keeper(), common.BNBAsset, node.BondAddress, node, nodeBond)
	...
	SetupLiquidityBondForTestV105(c, ctx, mgr.Keeper(), common.BNBAsset, node1.BondAddress, node1, node1Bond)
...
type TestSlashNodeAccountLPKeeperV110 struct {
	keeper.Keeper
	failCalcBond                     bool
	zeroBond                         bool
	failGetPolAddr                   bool
...
func (k *TestSlashNodeAccountLPKeeperV110) CalcNodeLiquidityBond(ctx cosmos.Context, na NodeAccount) (cosmos.Uint, error) {
	if k.failCalcBond {
...
func (k *TestSlashNodeAccountLPKeeperV110) GetModuleAddress(module string) (common.Address, error) {
	if k.failGetPolAddr {
...
func (k *TestSlashNodeAccountLPKeeperV110) GetBondProviders(ctx cosmos.Context, nodeAddr cosmos.AccAddress) (BondProviders, error) {
...
func (k *TestSlashNodeAccountLPKeeperV110) GetLiquidityProviderByAssets(ctx cosmos.Context, assets common.Assets, assetAddr common.Address) (LiquidityProviders, error) {
...
func (k *TestSlashNodeAccountLPKeeperV110) GetPool(ctx cosmos.Context, asset common.Asset) (Pool, error) {
	if k.failGetPool {
...
func (k *TestSlashNodeAccountLPKeeperV110) GetLiquidityProvider(ctx cosmos.Context, asset common.Asset, lpAddr common.Address) (LiquidityProvider, error) {
	if k.failGetLP {
...
func (s *SlashingV110Suite) TestSlashNodeAccountLP(c *C) {
	ctx, mgr := setupManagerForTest(c)
	keeper := &TestSlashNodeAccountLPKeeperV110{
		Keeper: mgr.Keeper(),
	}

	slasher := newSlasherV110(keeper, mgr.EventMgr())
	// when slash is zero
	...
	c.Assert(err, IsNil)
	add, err := common.NewAddress(GenesisNodes[0], GetCurrentVersion())
	c.Assert(err, IsNil)
	na := GetRandomValidatorNode(NodeActive)
	na.NodeAddress = acc
	na.BondAddress = add
	...
	SetupLiquidityBondForTestV105(c, ctx, keeper.Keeper, common.BTCAsset, na.BondAddress, na, nodeBond)
...
func (s *SlashingV110Suite) TestNetworkShouldNotSlashMorethanVaultAmount(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV110(mgr.Keeper(), mgr.EventMgr())
	...
	SetupLiquidityBondForTestV105(c, ctx, mgr.Keeper(), common.BTCAsset, node1.BondAddress, node1, node1Bond)
	acc, err = node1.BondAddress.AccAddress()
	c.Assert(err, IsNil)
	bp = NewBondProviders(node1.NodeAddress)
	bp.Providers = append(bp.Providers, BondProvider{
	...
	SetupLiquidityBondForTestV105(c, ctx, mgr.Keeper(), common.BTCAsset, node.BondAddress, node, cosmos.NewUint(1000*common.One))
...
func (s *SlashingV110Suite) TestNeedsNewVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
