...
func newPoolMgrVCUR(k keeper.Keeper) *PoolMgrVCUR {
	return &PoolMgrVCUR{
		keeper: k,
	}
}
...
func (pm *PoolMgrVCUR) EndBlock(ctx cosmos.Context, mgr Manager) error {
	...
	if ctx.BlockHeight()%poolCycle == 0 && !mgr.Keeper().RagnarokInProgress(ctx) {
		...
		minRunePoolDepth, err := mgr.Keeper().GetMimir(ctx, constants.MinCacaoPoolDepth.String())
		if minRunePoolDepth < 0 || err != nil {
			minRunePoolDepth = mgr.GetConstants().GetInt64Value(constants.MinCacaoPoolDepth)
		}
...
func (pm *PoolMgrVCUR) cyclePools(ctx cosmos.Context, maxAvailablePools, minRunePoolDepth, stagedPoolCost int64, mgr Manager) error {
	...
	validPool := func(pool Pool) bool {
		if pool.BalanceAsset.IsZero() || pool.BalanceCacao.IsZero() || pool.BalanceCacao.LT(minRuneDepth) {
			return false
		}
		return true
	}
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		case PoolAvailable:
			...
			if pool.BalanceCacao.LT(choppingBlock.BalanceCacao) || choppingBlock.IsEmpty() {
		...
		case PoolStaged:
			...
			if fee.GT(pool.BalanceCacao) {
				fee = pool.BalanceCacao
			}
			if !fee.IsZero() {
				pool.BalanceCacao = common.SafeSub(pool.BalanceCacao, fee)
			...
			} else if validPool(pool) && onDeck.BalanceCacao.LT(pool.BalanceCacao) {
				onDeck = pool
			}
		}
	}
	...
	if availblePoolCount >= maxAvailablePools {
		...
		if onDeck.BalanceCacao.LTE(choppingBlock.BalanceCacao) {
...
func (pm *PoolMgrVCUR) cleanupPendingLiquidity(ctx cosmos.Context, mgr Manager) {
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		if pool.PendingInboundCacao.IsZero() && pool.PendingInboundAsset.IsZero() {
			continue
		}
		if isChainHalted(ctx, mgr, pool.Asset.GetChain()) || isLPPaused(ctx, pool.Asset.GetChain(), mgr) {
			continue
		}
...
func (pm *PoolMgrVCUR) commitPendingLiquidity(ctx cosmos.Context, pool Pool, mgr Manager) error {
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		if lp.AssetAddress.IsEmpty() || lp.CacaoAddress.IsEmpty() {
			continue
		}
		...
		cacaoAdd := cosmos.ZeroUint()
		assetAdd := cosmos.ZeroUint()
		tx := common.Tx{
			ID:        common.BlankTxID,
			ToAddress: common.NoAddress,
			Memo:      "MAYA-PENDING-LIQUIDITY-ADD",
		}

		if !lp.PendingCacao.IsZero() {
		...
		if !lp.PendingAsset.IsZero() {
			tx.FromAddress = lp.CacaoAddress
			tx.Chain = common.BASEChain
			tx.Coins = common.NewCoins(common.NewCoin(common.BaseAsset(), cacaoAdd))
		}

		msg := NewMsgAddLiquidity(tx, lp.Asset, cacaoAdd, assetAdd, lp.CacaoAddress, lp.AssetAddress, common.NoAddress, cosmos.ZeroUint(), signer, 1)
		_, err := handler(ctx, msg)
		if err != nil {
			ctx.Logger().Error("failed to commit pending liquidity", "asset", lp.Asset, "maya address", lp.CacaoAddress, "asset address", lp.AssetAddress, "pending cacao", lp.PendingCacao, "pending asset", lp.PendingAsset, "error", err)
		...
		count += 1
		cleanedCacao = cleanedCacao.Add(lp.PendingCacao)
		cleanedAsset = cleanedCacao.Add(lp.PendingAsset)
	}
	ctx.Logger().Info("cleaned pending liquidity", "pool", pool.Asset, "count", count, "cleared rune", cleanedCacao.String(), "cleared asset", cleanedAsset.String())

	// add telemetry
	telemetry.IncrCounterWithLabels(
		[]string{"mayanode", "pool", "pending", "clear", "count"},
		float32(count),
		[]metrics.Label{telemetry.NewLabel("pool", pool.Asset.String())},
	)
	telemetry.IncrCounterWithLabels(
		[]string{"mayanode", "pool", "pending", "clear", "rune"},
		telem(cleanedCacao),
		[]metrics.Label{telemetry.NewLabel("pool", pool.Asset.String())},
	)
	telemetry.IncrCounterWithLabels(
		[]string{"mayanode", "pool", "pending", "clear", "asset"},
		telem(cleanedAsset),
		[]metrics.Label{telemetry.NewLabel("pool", pool.Asset.String())},
	)
