...
func (vts *validatorMgrVCURTestSuite) TestChurnWithChainHalt(c *C) {
	...
	SetupLiquidityBondForTest(c, ctx, mgr.Keeper(), common.BTCAsset, standbyNode.BondAddress, standbyNode, cosmos.NewUint(200000000))
...
func (vts *validatorMgrVCURTestSuite) TestEquitableBondReward(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *validatorMgrVCURTestSuite) TestFindNextVaultNodeAccountsBFT(c *C) {
	...
	err := validatorMgr.markReadyActors(ctx, constAccessor)
	c.Assert(err, IsNil)
	nasAfter, rotate, err := validatorMgr.nextVaultNodeAccounts(ctx, 4, constAccessor)
	c.Assert(err, IsNil)
