...
// NetworkMgrV118 is going to manage the vaults
type NetworkMgrV118 struct {
	k          keeper.Keeper
	txOutStore TxOutStore
	eventMgr   EventManager
}

// newNetworkMgrV118 create a new vault manager
func newNetworkMgrV118(k keeper.Keeper, txOutStore TxOutStore, eventMgr EventManager) *NetworkMgrV118 {
	return &NetworkMgrV118{
		k:          k,
		txOutStore: txOutStore,
		eventMgr:   eventMgr,
	}
...
func (vm *NetworkMgrV118) processGenesisSetup(ctx cosmos.Context) error {
...
func (vm *NetworkMgrV118) EndBlock(ctx cosmos.Context, mgr Manager) error {
	if ctx.BlockHeight() == genesisBlockHeight {
...
func (vm *NetworkMgrV118) paySaverYield(ctx cosmos.Context, asset common.Asset, runeAmt cosmos.Uint) error {
	...
	max, err := vm.k.GetMimir(ctx, constants.MaxSynthsForSaversYield.String())
	if max < 0 || err != nil {
		constAccessor := constants.GetConstantValues(vm.k.GetVersion())
		max = constAccessor.GetInt64Value(constants.MaxSynthsForSaversYield)
		if err != nil {
			ctx.Logger().Error("fail to fetch mimir value", "key", constants.SynthYieldBasisPoints.String(), "error", err)
			return err
		}
	...
	tx := common.NewTx(common.BlankTxID, modAddress, asgardAddress, common.NewCoins(coin), nil, "SAVERS-YIELD")
...
func (vm *NetworkMgrV118) POLCycle(ctx cosmos.Context, mgr Manager) error {
	maxDeposit := vm.k.GetConfigInt64(ctx, constants.POLMaxNetworkDeposit)
	movement := vm.k.GetConfigInt64(ctx, constants.POLMaxPoolMovement)
	target := vm.k.GetConfigInt64(ctx, constants.POLSynthUtilization)
	buf := vm.k.GetConfigInt64(ctx, constants.POLBuffer)
...
func (mv *NetworkMgrV118) fetchPOLPools(ctx cosmos.Context, mgr Manager) Pools {
	var pools Pools
...
func (vm *NetworkMgrV118) addPOLLiquidity(
	ctx cosmos.Context,
	pool Pool,
...
func (vm *NetworkMgrV118) removePOLLiquidity(
	ctx cosmos.Context,
	pool Pool,
	polAddress, asgardAddress common.Address,
	signer cosmos.AccAddress,
...
func (vm *NetworkMgrV118) TriggerKeygen(ctx cosmos.Context, nas NodeAccounts) error {
...
func (vm *NetworkMgrV118) RotateVault(ctx cosmos.Context, vault Vault) error {
	active, err := vm.k.GetAsgardVaultsByStatus(ctx, ActiveVault)
	if err != nil {
		return err
	}
...
func (vm *NetworkMgrV118) cleanupAsgardIndex(ctx cosmos.Context) error {
...
// // and ragnaroks them
// func (vm *NetworkMgrV118) manageChains(ctx cosmos.Context, mgr Manager) error {
...
// // ragnarok'ed
// func (vm *NetworkMgrV118) findChainsToRetire(ctx cosmos.Context) (common.Chains, error) {
...
func (vm *NetworkMgrV118) RecallChainFunds(ctx cosmos.Context, chain common.Chain, mgr Manager, excludeNodes common.PubKeys) error {
...
// func (vm *NetworkMgrV118) ragnarokChain(ctx cosmos.Context, chain common.Chain, nth int64, mgr Manager) error {
...
func (vm *NetworkMgrV118) withdrawLiquidity(ctx cosmos.Context, pool Pool, na NodeAccount, mgr Manager) error {
	if pool.Status == PoolSuspended {
...
func (vm *NetworkMgrV118) UpdateNetwork(ctx cosmos.Context, constAccessor constants.ConstantValues, gasManager GasManager, eventMgr EventManager) error {
	...
	mayaFundRewards := cosmos.ZeroUint()
	...
	if !tenPercentLiquidityFees.IsZero() && reserveBalance.GTE(tenPercentLiquidityFees) {
		...
		mayaFundRewards = tenPercentLiquidityFees
	}
	...
	// If CACAOPool rewards enabled move %5 liquidity rewards from Reserve to CACAOPool
	cacaoPoolRewards := cosmos.ZeroUint()

	// TODO: Remove version check and use only CACAOPoolRewardsEnabled after v1.123.0
	// Older versions (< 1.123.0) use CACAOPoolEnabled (disabled by default)
	// Newer versions (>= 1.123.0) use CACAOPoolRewardsEnabled (enabled by default)
	cacaoPoolRewardsEnabled := constAccessor.GetInt64Value(constants.CACAOPoolEnabled)
	if vm.k.GetVersion().GTE(semver.MustParse("1.123.0")) {
		// newer version uses CACAOPoolRewardsEnabled and enabled by default
		cacaoPoolRewardsEnabled = vm.k.GetConfigInt64(ctx, constants.CACAOPoolRewardsEnabled)
	}
	if cacaoPoolRewardsEnabled > 0 {
		// Move Cacao from the Reserve to the CACAOPool module
		fivePercentLiquidityFees := tenPercentLiquidityFees.QuoUint64(2)
		reserveBalance = vm.k.GetRuneBalanceOfModule(ctx, ReserveName)
		if !fivePercentLiquidityFees.IsZero() && reserveBalance.GTE(fivePercentLiquidityFees) {
			coin = common.NewCoin(common.BaseNative, fivePercentLiquidityFees)
			if err = vm.k.SendFromModuleToModule(ctx, ReserveName, CACAOPoolName, common.NewCoins(coin)); err != nil {
				ctx.Logger().Error("fail to transfer funds from Reserve to CACAOPool module", "error", err)
				return fmt.Errorf("fail to transfer funds from Reserve to CACAOPool module: %w", err)
			}
			var cacaoPool CACAOPool
			cacaoPool, err = vm.k.GetCACAOPool(ctx)
			if err != nil {
				return fmt.Errorf("fail to get CACAOPool: %w", err)
			}
			cacaoPool.CacaoDeposited = cacaoPool.CacaoDeposited.Add(fivePercentLiquidityFees)
			vm.k.SetCACAOPool(ctx, cacaoPool)
			cacaoPoolRewards = fivePercentLiquidityFees
		}
	}

	rewardEvt := NewEventRewards(bondReward, evtPools, cacaoPoolRewards, mayaFundRewards)
...
func (vm *NetworkMgrV118) getTotalProvidedLiquidityRune(ctx cosmos.Context) (Pools, cosmos.Uint, error) {
...
func (vm *NetworkMgrV118) getTotalActiveBond(ctx cosmos.Context) (cosmos.Uint, error) {
...
func (vm *NetworkMgrV118) calcPoolDeficit(lpDeficit, totalFees, poolFees cosmos.Uint) cosmos.Uint {
...
func (vm *NetworkMgrV118) calcBlockRewards(ctx cosmos.Context, totalProvidedLiquidity, totalBonded, totalLiquidityFees cosmos.Uint) (cosmos.Uint, cosmos.Uint) {
...
func (vm *NetworkMgrV118) deductPoolRewardDeficit(ctx cosmos.Context, pools Pools, totalLiquidityFees, lpDeficit cosmos.Uint) ([]PoolAmt, error) {
...
func (vm *NetworkMgrV118) checkPoolRagnarok(ctx cosmos.Context, mgr Manager) error {
...
func (vm *NetworkMgrV118) canRagnarokGasPool(ctx cosmos.Context, c common.Chain, allPools Pools) bool {
...
func (vm *NetworkMgrV118) redeemSynthAssetToReserve(ctx cosmos.Context, p Pool) error {
...
func (vm *NetworkMgrV118) ragnarokPool(ctx cosmos.Context, mgr Manager, p Pool) error {
