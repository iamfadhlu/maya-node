...
type TxOutStoreSuiteV104 struct{}

var _ = Suite(&TxOutStoreSuiteV104{})

func (s TxOutStoreSuiteV104) TestAddGasFees(c *C) {
	ctx, mgr := setupManagerForTest(c)
	tx := GetRandomObservedTx()

	version := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(version)
	mgr.gasMgr = newGasMgrV104(constAccessor, mgr.Keeper())
	err := addGasFees(ctx, mgr, tx)
	c.Assert(err, IsNil)
	c.Assert(mgr.GasMgr().GetGas(), HasLen, 1)
}

func (s TxOutStoreSuiteV104) TestEndBlock(c *C) {
...
func (s TxOutStoreSuiteV104) TestAddOutTxItem(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
	...
	version := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(version)
	txOutStore := newTxOutStorageV104(w.keeper, w.mgr.GetConstants(), w.mgr.EventMgr(), newGasMgrV104(constAccessor, w.keeper))
...
func (s TxOutStoreSuiteV104) TestAddOutTxItem_OutboundHeightDoesNotGetOverride(c *C) {
	SetupConfigForTest()
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
...
func (s TxOutStoreSuiteV104) TestAddOutTxItemNotEnoughForFee(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
		common.NewCoin(common.BaseAsset(), cosmos.NewUint(10000*common.One)),
...
func (s TxOutStoreSuiteV104) TestAddOutTxItemDeductMaxGasFromYggdrasil(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
		common.NewCoin(common.BaseAsset(), cosmos.NewUint(10000*common.One)),
...
func (s TxOutStoreSuiteV104) TestcalcTxOutHeight(c *C) {
...
func (s TxOutStoreSuiteV104) TestAddOutTxItem_MultipleOutboundWillBeScheduledAtTheSameBlockHeight(c *C) {
	SetupConfigForTest()
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
...
func (s TxOutStoreSuiteV104) TestAddOutTxItemInteractionWithPool(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
...
func (s TxOutStoreSuiteV104) TestAddOutTxItemSendingFromRetiredVault(c *C) {
	SetupConfigForTest()
	w := getHandlerTestWrapper(c, 1, true, true)
