...
func newTssHandlerTestHelper(c *C) tssHandlerTestHelper {
	...
	SetupLiquidityBondForTestV105(c, ctx, k, common.BNBAsset, nodeAccount.BondAddress, nodeAccount, cosmos.NewUint(100*common.One))
	...
	nodeReadyBond := cosmos.NewUint(1000000 * common.One)
	bp = NewBondProviders(nodeReady.NodeAddress)
	acc, err = nodeAccount.BondAddress.AccAddress()
	c.Assert(err, IsNil)
	bp.Providers = append(bp.Providers, NewBondProvider(acc))
	bp.Providers[0].Bonded = true
	SetupLiquidityBondForTest(c, ctx, k, common.BNBAsset, nodeAccount.BondAddress, nodeReady, nodeReadyBond)
	c.Assert(k.SetBondProviders(ctx, bp), IsNil)
	...
	msg, err := NewMsgTssPoolV2(members.Strings(), poolPk, fakeSig, AsgardKeygen, ctx.BlockHeight(), nil, common.Chains{common.BaseAsset().Chain}.Strings(), signer, keygenTime, poolPkEddsa, []byte("backup"))
	c.Assert(err, IsNil)
	voter := NewTssVoter(msg.ID, members.Strings(), poolPk, poolPkEddsa)
	keeperHelper.SetTssVoter(ctx, voter)

	asgardVault := NewVaultV2(ctx.BlockHeight(), ActiveVault, AsgardVault, GetRandomPubKey(), common.Chains{common.BaseAsset().Chain}.Strings(), []ChainContract{}, poolPkEddsa)
...
func (s *HandlerTssSuite) TestTssHandler(c *C) {
	...
	testCases := []struct {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				msg, err := NewMsgTssPool(helper.members.Strings(), GetRandomPubKey(), nil, AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), GetRandomValidatorNode(NodeActive).NodeAddress, keygenTime)
				c.Assert(err, IsNil)
				return msg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				msg, err := NewMsgTssPool(helper.members.Strings(), GetRandomPubKey(), nil, AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), cosmos.AccAddress{}, keygenTime)
				c.Assert(err, IsNil)
				return msg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPool(helper.members.Strings(), GetRandomPubKey(), nil, AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				tssMsg.ID = ""
				return tssMsg
			},
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPool(nil, GetRandomPubKey(), nil, AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				return tssMsg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPool(common.PubKeys{GetRandomPubKey()}.Strings(), GetRandomPubKey(), nil, AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				return tssMsg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPool([]string{GetRandomPubKey().String(), GetRandomPubKey().String(), ""}, GetRandomPubKey(), nil, AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				return tssMsg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPool(helper.members.Strings(), common.EmptyPubKey, nil, AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				return tssMsg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPool(helper.members.Strings(), "whatever", nil, AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				return tssMsg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPoolV2(helper.members.Strings(), GetRandomPubKey(), fakeSig, AsgardKeygen, helper.ctx.BlockHeight(), nil, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime, GetRandomPubKey(), []byte("backup"))
				c.Assert(err, IsNil)
				return tssMsg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPoolV2(helper.members.Strings(), helper.poolPk, fakeSig, AsgardKeygen, helper.ctx.BlockHeight(), nil, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime, helper.poolPkEddsa, []byte("backup"))
				c.Assert(err, IsNil)
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				...
				tssMsg, err := NewMsgTssPoolV2(helper.members.Strings(), GetRandomPubKey(), fakeSig, AsgardKeygen, helper.ctx.BlockHeight(), []Blame{b}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime, GetRandomPubKey(), []byte("backup"))
				c.Assert(err, IsNil)
				voter, err := helper.keeper.GetTssVoter(helper.ctx, tssMsg.ID)
				c.Assert(err, IsNil)
				for _, pk := range helper.members {
		...
		},
		{
			...
			validator: func(helper tssHandlerTestHelper, msg cosmos.Msg, result *cosmos.Result, c *C) {
				...
				c.Assert(err, IsNil)
				c.Assert(naBond.Equal(cosmos.ZeroUint()), Equals, true)
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				...
				tssMsg, err := NewMsgTssPoolV2(members.Strings(), GetRandomPubKey(), fakeSig, AsgardKeygen, helper.ctx.BlockHeight(), nil, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime, GetRandomPubKey(), []byte("backup"))
				c.Assert(err, IsNil)
				return tssMsg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
	...
	for _, tc := range testCases {
		...
		mgr := NewDummyMgrWithKeeper(helper.keeper)
		mgr.slasher = newSlasherVCUR(mgr.K, mgr.eventMgr)
...
func (s *HandlerTssSuite) TestKeygenSuccessHandler(c *C) {
	...
	for idx, item := range helper.members {
		...
		c.Assert(err, IsNil)
		tssMsg, err := NewMsgTssPoolV2(helper.members.Strings(), poolPubKey, nil, AsgardKeygen, helper.ctx.BlockHeight(), nil, common.Chains{common.BaseAsset().Chain}.Strings(), thorAddr, keygenTime, poolEddsaPubKey, []byte("backup"))
		c.Assert(err, IsNil)
