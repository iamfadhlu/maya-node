...
func (s *SlasherVCUR) LackSigning(ctx cosmos.Context, mgr Manager) error {
	...
	for i, tx := range txs.TxArray {
		...
		if tx.OutHash.IsEmpty() {
			...
			if !frozen && s.needsNewVault(ctx, mgr, vault, signingTransPeriod, voter.FinalisedHeight, tx) {
				var active Vaults
			...
			if !tx.Coin.Asset.IsBase() && !s.keeper.PoolExist(ctx, tx.Coin.Asset) {
				ctx.Logger().Error("fail to add outbound to queue", "error", "coin is not cacao and does not have an associated pool")
				continue
			}
...
func (s *SlasherVCUR) needsNewVault(ctx cosmos.Context, mgr Manager, vault Vault, signingTransPeriod, startHeight int64, tx TxOutItem) bool {
	outhashes := mgr.Keeper().GetObservedLink(ctx, tx.InHash)
	if len(outhashes) == 0 {
		return true
	}
	...
	for _, hash := range outhashes {
		...
		for _, tx1 := range voter.Txs {
			if tx1.ObservedPubKey.Equals(tx.VaultPubKey) {
				if !tx1.ObservedPubKey.Equals(tx.VaultPubKey) {
					if len(tx1.Tx.Coins) != 1 || !tx1.Tx.Coins[0].Asset.Equals(tx.Coin.Asset) {
						continue
					}
				}
		...
		if len(signers) > 0 {
			...
			for _, member := range vault.Membership {
				...
				addy, err := pk.GetAddress(common.BASEChain)
				if err != nil {
					ctx.Logger().Error("fail to get address from pubkey", "error", err)
					continue
				}

				if _, ok := signers[addy.String()]; ok {
					count++
				}
			}
