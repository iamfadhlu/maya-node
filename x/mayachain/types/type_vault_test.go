...
func (s *VaultSuite) TestDeductVaultPendingOutbounds(c *C) {
	vault := NewVault(12, VaultStatus_ActiveVault, VaultType_AsgardVault, GetRandomPubKey(), common.Chains{common.BNBChain}.Strings(), []ChainContract{})
	...
	pendingTxs := []TxOutItem{
		{
			VaultPubKey: vault.PubKey,
			Coin:        common.NewCoin(common.BNBAsset, cosmos.NewUint(40*common.One)),
	...
	vault.DeductVaultPendingOutbounds(pendingTxs)

	c.Check(vault.GetCoin(common.BNBAsset).Amount.Equal(cosmos.NewUint(59*common.One)), Equals, true)
	c.Check(vault.GetCoin(common.BTCAsset).Amount.Equal(cosmos.NewUint(39*common.One)), Equals, true)
}
