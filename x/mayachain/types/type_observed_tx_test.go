...
func (s TypeObservedTxSuite) TestVoter(c *C) {
	...
	thorchainCoins := common.Coins{
		common.NewCoin(common.BaseAsset(), cosmos.NewUint(100)),
...
func (TypeObservedTxSuite) TestObservedTxEquals(c *C) {
	...
	bnb, err := common.NewAddress("bnb1xlvns0n2mxh77mzaspn2hgav4rr4m8eerfju38", GetCurrentVersion())
	c.Assert(err, IsNil)
	bnb1, err := common.NewAddress("bnb1yk882gllgv3rt2rqrsudf6kn2agr94etnxu9a7", GetCurrentVersion())
	c.Assert(err, IsNil)
	observePoolAddr := GetRandomPubKey()
