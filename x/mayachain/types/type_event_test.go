...
func (s EventSuite) TestAffiliateFee(c *C) {
	evt := NewEventAffiliateFee(
		GetRandomTxHash(),
		"memo",
		"mayaname",
	...
	c.Check(evt.Type(), Equals, "affiliate_fee")
	events, err := evt.Events()
	c.Check(err, IsNil)
	c.Check(events, NotNil)
}
...
func (s EventSuite) TestReward(c *C) {
	...
	c.Check(evt.CacaoPoolReward.String(), Equals, "100")
	c.Check(evt.MayaFundReward.String(), Equals, "100")
	events, err := evt.Events()
	c.Check(err, IsNil)
	c.Check(events, NotNil)
}
