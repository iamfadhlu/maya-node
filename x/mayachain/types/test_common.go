...
// GetRandomTHORAddress will just create a random thorchain address used for test purpose
...
func GetRandomAZTECAddress() common.Address {
	...
	aztec, _ := common.NewAddress(str, GetCurrentVersion())
	return aztec
}
...
func GetRandomBNBAddress() common.Address {
	...
	bnb, _ := common.NewAddress(str, GetCurrentVersion())
	return bnb
}
...
func GetRandomKUJIAddress() common.Address {
	name := common.RandHexString(10)
	str, _ := common.ConvertAndEncode("kujira", crypto.AddressHash([]byte(name)))
	kuji, _ := common.NewAddress(str, GetCurrentVersion())
	return kuji
}
...
func GetRandomXRDAddress() common.Address {
	pubKey := GetRandomPubKey()
	addr, _ := pubKey.GetAddress(common.XRDChain)
	return addr
}

func GetRandomZECAddress() common.Address {
	pubKey := GetRandomPubKey()
	addr, _ := pubKey.GetAddress(common.ZECChain)
	return addr
}
