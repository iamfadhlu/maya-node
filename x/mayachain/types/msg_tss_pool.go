...
// NewMsgTssPool is a constructor function for MsgTssPool (pre-v124)
func NewMsgTssPool(pks []string, poolpk common.PubKey, keysharesBackup []byte, keygenType KeygenType, height int64, blame Blame, chains []string, signer cosmos.AccAddress, keygenTime int64) (*MsgTssPool, error) {
	id, err := getTssID(pks, poolpk, height, blame)
	if err != nil {
...
func (m *MsgTssPool) ValidateBasic() error {
	...
	if !chains.Has(common.BaseAsset().Chain) {
