...
type SlashingV113Suite struct{}

var _ = Suite(&SlashingV113Suite{})

func (s *SlashingV113Suite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (s *SlashingV113Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := newSlasherV113(k, NewDummyEventMgr())
...
func (s *SlashingV113Suite) TestLackObservingErrors(c *C) {
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV113(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV113Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
...
func (s *SlashingV113Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV113Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomValidatorNode(NodeActive),
		GetRandomValidatorNode(NodeActive),
	}
...
func (s *SlashingV113Suite) TestDoubleSign(c *C) {
...
func (s *SlashingV113Suite) TestMissingSign(c *C) {
	ctx, mgr := setupManagerForTest(c)
	constAccessor := constants.GetConstantValues(GetCurrentVersion())

	na := GetRandomValidatorNode(NodeActive)
	c.Assert(mgr.Keeper().SetNodeAccount(ctx, na), IsNil)
	slasher := newSlasherV113(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV113Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV113Suite) TestSlashVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV113(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV113Suite) TestSlashNodeAccountLP(c *C) {
	ctx, mgr := setupManagerForTest(c)
	keeper := &TestSlashNodeAccountLPKeeper{
		Keeper: mgr.Keeper(),
	}

	slasher := newSlasherV113(keeper, mgr.EventMgr())
	// when slash is zero
...
func (s *SlashingV113Suite) TestNetworkShouldNotSlashMorethanVaultAmount(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV113(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV113Suite) TestNeedsNewVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
