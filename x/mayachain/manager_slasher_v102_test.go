...
type SlashingV102Suite struct{}

var _ = Suite(&SlashingV102Suite{})

type TestSlashingLackKeeperV102 struct {
	keeper.KVStoreDummy
	txOut                      *TxOut
	na                         NodeAccount
	vaults                     Vaults
...
func (k *TestSlashingLackKeeperV102) PoolExist(ctx cosmos.Context, asset common.Asset) bool {
	return k.pools[asset].IsEmpty()
}

func (k *TestSlashingLackKeeperV102) GetObservedTxInVoter(_ cosmos.Context, _ common.TxID) (ObservedTxVoter, error) {
...
func (k *TestSlashingLackKeeperV102) SetObservedTxInVoter(_ cosmos.Context, voter ObservedTxVoter) {
	k.voter = voter
}

func (k *TestSlashingLackKeeperV102) GetVault(_ cosmos.Context, pk common.PubKey) (Vault, error) {
	if k.failGetVault {
...
func (k *TestSlashingLackKeeperV102) GetAsgardVaultsByStatus(_ cosmos.Context, _ VaultStatus) (Vaults, error) {
...
func (k *TestSlashingLackKeeperV102) GetTxOut(_ cosmos.Context, _ int64) (*TxOut, error) {
	if k.failGetTxOut {
		return nil, errKaboom
	}
	return k.txOut, nil
...
func (k *TestSlashingLackKeeperV102) SetTxOut(_ cosmos.Context, tx *TxOut) error {
	if k.failSetTxOut {
		return errKaboom
	}
	k.txOut = tx
...
func (k *TestSlashingLackKeeperV102) IncNodeAccountSlashPoints(_ cosmos.Context, addr cosmos.AccAddress, pts int64) error {
...
func (k *TestSlashingLackKeeperV102) GetNodeAccountByPubKey(_ cosmos.Context, _ common.PubKey) (NodeAccount, error) {
...
func (k *TestSlashingLackKeeperV102) SetNodeAccount(_ cosmos.Context, na NodeAccount) error {
...
type TestSlashObservingKeeperV102 struct {
	keeper.KVStoreDummy
...
func (k *TestSlashObservingKeeperV102) GetObservingAddresses(_ cosmos.Context) ([]cosmos.AccAddress, error) {
...
func (k *TestSlashObservingKeeperV102) ClearObservingAddresses(_ cosmos.Context) {
	k.addrs = nil
}

func (k *TestSlashObservingKeeperV102) IncNodeAccountSlashPoints(_ cosmos.Context, addr cosmos.AccAddress, pts int64) error {
	if _, ok := k.slashPts[addr.String()]; !ok {
		k.slashPts[addr.String()] = 0
	}
	k.slashPts[addr.String()] += pts
...
func (k *TestSlashObservingKeeperV102) ListActiveValidators(_ cosmos.Context) (NodeAccounts, error) {
...
func (k *TestSlashObservingKeeperV102) SetNodeAccount(_ cosmos.Context, na NodeAccount) error {
	if k.failSetNodeAccount {
		return errKaboom
	}
	for i := range k.nas {
...
type TestDoubleSlashKeeperV102 struct {
	keeper.KVStoreDummy
	na          NodeAccount
	naBond      cosmos.Uint
	bp          BondProviders
...
func (s *SlashingV102Suite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (s *SlashingV102Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := newSlasherV102(k, NewDummyEventMgr())
...
func (s *SlashingV102Suite) TestLackObservingErrors(c *C) {
	...
	keeper := &TestSlashObservingKeeperV102{
		nas:      nas,
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV102(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV102Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
		condition   func(keeper *TestSlashingLackKeeperV102)
		shouldError bool
	}{
		{
	...
	for _, item := range testCases {
		...
		keeper := &TestSlashingLackKeeperV102{
			txOut:  txOut,
			na:     na,
			vaults: Vaults{ygg},
			voter: ObservedTxVoter{
...
func (s *SlashingV102Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV102Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomValidatorNode(NodeActive),
		GetRandomValidatorNode(NodeActive),
	}
...
func (s *SlashingV102Suite) TestDoubleSign(c *C) {
	...
	slasher := newSlasherV102(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV102Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
	...
	keeper := &TestDoubleSlashKeeperV102{
		na:     na,
		naBond: naBond,
		bp:     bp,
		lp: LiquidityProvider{
...
func (s *SlashingV102Suite) TestSlashVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV102(mgr.Keeper(), mgr.EventMgr())
...
type TestSlashNodeAccountLPKeeperV102 struct {
	keeper.Keeper
	failCalcBond                     bool
	zeroBond                         bool
	failGetPolAddr                   bool
...
func (k *TestSlashNodeAccountLPKeeperV102) CalcNodeLiquidityBond(ctx cosmos.Context, na NodeAccount) (cosmos.Uint, error) {
	if k.failCalcBond {
...
func (k *TestSlashNodeAccountLPKeeperV102) GetModuleAddress(module string) (common.Address, error) {
	if k.failGetPolAddr {
...
func (k *TestSlashNodeAccountLPKeeperV102) GetBondProviders(ctx cosmos.Context, nodeAddr cosmos.AccAddress) (BondProviders, error) {
...
func (k *TestSlashNodeAccountLPKeeperV102) GetLiquidityProviderByAssets(ctx cosmos.Context, assets common.Assets, assetAddr common.Address) (LiquidityProviders, error) {
...
func (k *TestSlashNodeAccountLPKeeperV102) GetPool(ctx cosmos.Context, asset common.Asset) (Pool, error) {
	if k.failGetPool {
...
func (k *TestSlashNodeAccountLPKeeperV102) GetLiquidityProvider(ctx cosmos.Context, asset common.Asset, lpAddr common.Address) (LiquidityProvider, error) {
	if k.failGetLP {
...
func (s *SlashingV102Suite) TestSlashNodeAccountLP(c *C) {
	ctx, mgr := setupManagerForTest(c)
	keeper := &TestSlashNodeAccountLPKeeperV102{
		Keeper: mgr.Keeper(),
	}

	slasher := newSlasherV102(keeper, mgr.EventMgr())
	// when slash is zero
...
func (s *SlashingV102Suite) TestNetworkShouldNotSlashMorethanVaultAmount(c *C) {
	ctx, mgr := setupManagerForTest(c)
	slasher := newSlasherV102(mgr.Keeper(), mgr.EventMgr())
...
func (s *SlashingV102Suite) TestNeedsNewVault(c *C) {
	ctx, mgr := setupManagerForTest(c)
