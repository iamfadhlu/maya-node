...
func (p *parser) ParseYggdrasilFundMemo() (YggdrasilFundMemo, error) {
	...
	case p.version.GTE(semver.MustParse("1.112.0")):
		return p.ParseYggdrasilFundMemoV112()
	default:
...
func (p *parser) ParseYggdrasilReturnMemo() (YggdrasilReturnMemo, error) {
	switch {
	case p.version.GTE(semver.MustParse("1.112.0")):
		return p.ParseYggdrasilReturnMemoV112()
	default:
		return ParseYggdrasilReturnMemoV1(p.parts)
	}
}

func (p *parser) ParseYggdrasilFundMemoV112() (YggdrasilFundMemo, error) {
	blockHeight := p.getInt64(1, true, 0)
	return NewYggdrasilFund(blockHeight), p.Error()
}

func (p *parser) ParseYggdrasilReturnMemoV112() (YggdrasilReturnMemo, error) {
	blockHeight := p.getInt64(1, true, 0)
