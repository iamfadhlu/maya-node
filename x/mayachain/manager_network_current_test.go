...
func (s *NetworkManagerVCURTestSuite) TestUpdateNetwork(c *C) {
	...
	helper.Keeper.SetMimir(ctx, constants.CACAOPoolRewardsEnabled.String(), 0)
...
func (s *NetworkManagerVCURTestSuite) TestSaverYield(c *C) {
	var err error
	ctx, mgr := setupManagerForTest(c)
...
func (s *NetworkManagerVCURTestSuite) TestCalcSynthYield(c *C) {
	ctx, mgr := setupManagerForTest(c)
	// net := newNetworkMgrVCUR(mgr.Keeper(), mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerVCURTestSuite) TestCleanupAsgardIndex(c *C) {
	ctx, k := setupKeeperForTest(c)
	vault1 := NewVault(1084, ActiveVault, AsgardVault, GetRandomPubKey(), common.Chains{common.BNBChain}.Strings(), []ChainContract{})
	c.Assert(k.SetVault(ctx, vault1), IsNil)
	vault2 := NewVault(1084, RetiringVault, AsgardVault, GetRandomPubKey(), common.Chains{common.BNBChain}.Strings(), []ChainContract{})
	c.Assert(k.SetVault(ctx, vault2), IsNil)
	vault3 := NewVault(1084, InitVault, AsgardVault, GetRandomPubKey(), common.Chains{common.BNBChain}.Strings(), []ChainContract{})
	c.Assert(k.SetVault(ctx, vault3), IsNil)
	vault4 := NewVault(1084, InactiveVault, AsgardVault, GetRandomPubKey(), common.Chains{common.BNBChain}.Strings(), []ChainContract{})
