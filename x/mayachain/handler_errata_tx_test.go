...
func (k *TestErrataTxKeeper) GetLiquidityProvider(_ cosmos.Context, asset common.Asset, addr common.Address) (LiquidityProvider, error) {
	for _, lp := range k.lps {
		if lp.CacaoAddress.Equals(addr) {
			return lp, k.err
		}
	}
...
func (k *TestErrataTxKeeper) SetLiquidityProvider(_ cosmos.Context, lp LiquidityProvider) {
	for i, skr := range k.lps {
		if skr.CacaoAddress.Equals(lp.CacaoAddress) {
			k.lps[i] = lp
		}
	}
}
...
func (s *HandlerErrataTxSuite) TestErrataHandlerHappyPath(c *C) {
	...
	observedTx := ObservedTx{
		Tx: common.Tx{
			...
			Memo: fmt.Sprintf("ADD:BNB.BNB:%s", GetRandomBaseAddress()),
		},
	}
	keeper := &TestErrataTxKeeper{
		na: na,
	...
	c.Assert(err, IsNil)
	c.Check(keeper.pool.BalanceCacao.Equal(cosmos.NewUint(70*common.One)), Equals, true)
		...
		},
		{
			...
			messageProvider: func(ctx cosmos.Context, helper *ErrataTxHandlerTestHelper) cosmos.Msg {
				...
				lp := LiquidityProvider{
					...
					LastAddHeight: 1024,
					CacaoAddress:  observedTx.Tx.FromAddress,
				}
				helper.SetLiquidityProvider(ctx, lp)
				helper.failGetLiquidityProvider = true
				pool := NewPool()
				pool.Asset = common.BTCAsset
				pool.BalanceCacao = cosmos.NewUint(common.One * 100)
...
func (*HandlerErrataTxSuite) TestProcessErrataOutboundTx(c *C) {
	...
	txID := GetRandomTxHash()
	msg := NewMsgErrataTx(txID, common.DASHChain, node1.NodeAddress)
	result, err := handler.processErrataOutboundTx(ctx, *msg)
	c.Assert(err, NotNil)
	c.Assert(result, IsNil)

	observedPubKey := GetRandomPubKey()
	tx := common.NewTx(txID, GetRandomDASHAddress(), GetRandomDASHAddress(),
		common.Coins{
			common.NewCoin(common.DASHAsset, cosmos.NewUint(102400000)),
		},
		common.Gas{
	...
	pool := NewPool()
	pool.Asset = common.DASHAsset
	pool.BalanceAsset = cosmos.NewUint(1024 * common.One)
	pool.BalanceCacao = cosmos.NewUint(1024 * common.One)
	pool.Status = PoolAvailable
	c.Assert(helper.Keeper.SetPool(ctx, pool), IsNil)
	result, err = handler.processErrataOutboundTx(ctx, *msg)
	c.Assert(err, IsNil)
	...
	txInbound := common.NewTx(txInID, GetRandomDASHAddress(), GetRandomDASHAddress(),
		common.Coins{
			common.NewCoin(common.DASHAsset, cosmos.NewUint(102400000)),
		},
		common.Gas{
	...
	newActiveAsgardVault.AddFunds(common.Coins{
		common.NewCoin(common.DASHAsset, cosmos.NewUint(1024*common.One)),
	})
	c.Assert(helper.Keeper.SetVault(ctx, newActiveAsgardVault), IsNil)
	c.Assert(helper.Keeper.SaveNetworkFee(ctx, common.DASHChain, NetworkFee{
...
func (*HandlerErrataTxSuite) TestProcessErrataOutboundTx_EnsureMigrateTxWillSetInactiveVaultBackToRetiring(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewErrataTxHandlerTestHelper(mgr.Keeper())
	mgr.K = helper
	handler := NewErrataTxHandler(mgr)
	...
	internalMigrationTx := NewMsgErrataTx(txID1, common.DASHChain, node1.NodeAddress)

	migrateTx := common.NewTx(txID1,
		GetRandomDASHAddress(),
		GetRandomDASHAddress(),
		common.Coins{
			common.NewCoin(common.DASHAsset, cosmos.NewUint(102400000)),
		},
	...
	DASHCoin := v.GetCoin(common.DASHAsset)
	c.Assert(DASHCoin.Equals(common.NewCoin(common.DASHAsset, cosmos.NewUint(102400000))), Equals, true)

	// reset the inactive vault
	c.Assert(helper.Keeper.SetVault(ctx, inactiveVault), IsNil)
	errataVoter := NewErrataTxVoter(txID1, common.DASHChain)
