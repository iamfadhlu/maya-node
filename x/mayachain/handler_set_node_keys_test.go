...
func (k *TestSetNodeKeysKeeper) HasCoins(ctx cosmos.Context, addr cosmos.AccAddress, coins cosmos.Coins) bool {
	return k.ensure != errNotEnoughBalance
}

func (k *TestSetNodeKeysKeeper) GetBondProviders(ctx sdk.Context, add sdk.AccAddress) (types.BondProviders, error) {
	return k.bps, nil
}

func (k *TestSetNodeKeysKeeper) GetLiquidityProviderByAssets(ctx cosmos.Context, assets common.Assets, addr common.Address) (LiquidityProviders, error) {
	return k.lps, nil
}

func (k *TestSetNodeKeysKeeper) GetPool(ctx sdk.Context, asset common.Asset) (types.Pool, error) {
	return k.pool, nil
}
...
func (s *HandlerSetNodeKeysSuite) TestValidate(c *C) {
	...
	// not enough balance
	keeper.ensure = fmt.Errorf("not enough balance")
	msg = NewMsgSetNodeKeys(keeper.na.PubKeySet, consensPubKey, keeper.na.NodeAddress)
	err = handler.validate(ctx, *msg)
	c.Assert(err, NotNil)
}
...
func (s *HandlerSetNodeKeysSuite) TestHandle(c *C) {
	...
	nodeAccount := NewNodeAccount(signer, NodeActive, emptyPubKeySet, bepConsPubKey, "", bond, bondAddr, ctx.BlockHeight())
	c.Assert(helper.Keeper.SetNodeAccount(ctx, nodeAccount), IsNil)

	nodeAccount = NewNodeAccount(signer, NodeWhiteListed, emptyPubKeySet, bepConsPubKey, "", bond, bondAddr, ctx.BlockHeight())
	c.Assert(helper.Keeper.SetNodeAccount(ctx, nodeAccount), IsNil)
