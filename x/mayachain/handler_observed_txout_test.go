...
func (k *TestObservedTxOutHandleKeeper) SetBondProviders(ctx cosmos.Context, bp BondProviders) error {
	k.bp = bp
	return nil
}

func (k *TestObservedTxOutHandleKeeper) GetBondProviders(ctx cosmos.Context, add cosmos.AccAddress) (BondProviders, error) {
	return k.bp, nil
}

func (k *TestObservedTxOutHandleKeeper) SetLiquidityProvider(_ cosmos.Context, lp LiquidityProvider) {
	k.lp = lp
}

func (k *TestObservedTxOutHandleKeeper) GetLiquidityProvider(_ cosmos.Context, asset common.Asset, addr common.Address) (LiquidityProvider, error) {
	return k.lp, nil
}

func (k *TestObservedTxOutHandleKeeper) GetLiquidityProviderByAssets(_ cosmos.Context, assets common.Assets, addr common.Address) (LiquidityProviders, error) {
	return LiquidityProviders{k.lp}, nil
}

func (k *TestObservedTxOutHandleKeeper) CalcNodeLiquidityBond(_ cosmos.Context, _ NodeAccount) (cosmos.Uint, error) {
	return k.naBond, nil
}

func (k *TestObservedTxOutHandleKeeper) SetLiquidityProviders(ctx cosmos.Context, lps LiquidityProviders) {
	for _, lp := range lps {
...
func (k *TestObservedTxOutHandleKeeper) GetModuleAddress(module string) (common.Address, error) {
	return GetRandomBaseAddress(), nil
}

func (k *TestObservedTxOutHandleKeeper) GetConfigInt64(ctx cosmos.Context, key constants.ConstantName) int64 {
	if key == constants.SlashPenalty {
		return constants.GetConstantValues(semver.MustParse("9999999.0.0")).GetInt64Value(constants.SlashPenalty)
	}
	if key == constants.PauseOnSlashThreshold {
		return constants.GetConstantValues(semver.MustParse("9999999.0.0")).GetInt64Value(constants.PauseOnSlashThreshold)
	}
	return -1
}
...
func (s *HandlerObservedTxOutSuite) TestHandle(c *C) {
	...
	ygg.Coins = common.Coins{
		common.NewCoin(common.BaseAsset(), cosmos.NewUint(500)),
	...
	keeper := &TestObservedTxOutHandleKeeper{
		...
		pool: Pool{
			...
			BalanceCacao: cosmos.NewUint(200_000),
...
func (s *HandlerObservedTxOutSuite) TestHandleFailedTransaction(c *C) {
	...
	mgr.GasMgr().EndBlock(ctx, keeper, eventMgr)
	c.Check(int(keeper.pool.BalanceAsset.Uint64()), Equals, 262_500)
	// As this was a failed transaction, Gas 37500 (being slashed) subtracted from the pool.
	mgr.GasMgr().EndBlock(ctx, keeper, eventMgr)
	c.Check(int(keeper.pool.BalanceAsset.Uint64()), Equals, 262_500)
...
func (s *HandlerObservedTxOutSuite) TestHandleStolenFundsInvalidMemo(c *C) {
	...
	lp.Bond(na.NodeAddress, cosmos.NewUint(1000*common.One))
	...
	expectedUnits := cosmos.NewUint(84999943750)
