...
func ValidateGenesis(data GenesisState) error {
	...
	for _, afc := range data.AffiliateCollectors {
		if err := afc.Valid(); err != nil {
			return fmt.Errorf("invalid affiliate collector: %w", err)
		}
	}
...
func DefaultGenesisState() GenesisState {
	return GenesisState{
		...
		StreamingSwaps:      make(StreamingSwaps, 0),
		...
		CacaoProviders:      make([]CACAOProvider, 0),
		CacaoPool:           NewCACAOPool(),
	}
}
...
func initGenesis(ctx cosmos.Context, keeper keeper.Keeper, data GenesisState) []abci.ValidatorUpdate {
	...
	for _, item := range data.SwapQueueItems {
		index := 0
		if keeper.HasSwapQueueItem(ctx, item.Tx.ID, 0) {
			index = 1
		}

		if err := keeper.SetSwapQueueItem(ctx, item, index); err != nil {
			panic(err)
		}
	}
	...
	// Mint coins into the Affiliate collector module
	if data.AffiliateCollectorModule > 0 {
		coin := common.NewCoin(common.BaseNative, cosmos.NewUint(data.AffiliateCollectorModule))
		if err := keeper.MintToModule(ctx, ModuleName, coin); err != nil {
			panic(err)
		}
		if err := keeper.SendFromModuleToModule(ctx, ModuleName, AffiliateCollectorName, common.NewCoins(coin)); err != nil {
			panic(err)
		}
	}
