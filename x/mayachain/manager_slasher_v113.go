...
// SlasherV113 is v88 implementation of slasher
type SlasherV113 struct {
	keeper   keeper.Keeper
	eventMgr EventManager
}

// newSlasherV113 create a new instance of Slasher
func newSlasherV113(keeper keeper.Keeper, eventMgr EventManager) *SlasherV113 {
	return &SlasherV113{keeper: keeper, eventMgr: eventMgr}
}

// BeginBlock called when a new block get proposed to detect whether there are duplicate vote
func (s *SlasherV113) BeginBlock(ctx cosmos.Context, req abci.RequestBeginBlock, constAccessor constants.ConstantValues) {
...
func (s *SlasherV113) HandleDoubleSign(ctx cosmos.Context, addr crypto.Address, infractionHeight int64, constAccessor constants.ConstantValues, validatorAddresses []nodeAddressValidatorAddressPair) error {
	...
	doubleBlockSignSlashPoints, err := s.keeper.GetMimir(ctx, constants.DoubleBlockSignSlashPoints.String())
	if doubleBlockSignSlashPoints < 0 || err != nil {
		doubleBlockSignSlashPoints = constAccessor.GetInt64Value(constants.DoubleBlockSignSlashPoints)
	}
...
func (s *SlasherV113) HandleMissingSign(ctx cosmos.Context, addr crypto.Address, constAccessor constants.ConstantValues, validatorAddresses []nodeAddressValidatorAddressPair) error {
	missBlockSignSlashPoints, err := s.keeper.GetMimir(ctx, constants.MissBlockSignSlashPoints.String())
	if missBlockSignSlashPoints < 0 || err != nil {
...
func (s *SlasherV113) LackObserving(ctx cosmos.Context, constAccessor constants.ConstantValues) error {
	signingTransPeriod, err := s.keeper.GetMimir(ctx, constants.SigningTransactionPeriod.String())
	if signingTransPeriod < 0 || err != nil {
...
func (s *SlasherV113) slashNotObserving(ctx cosmos.Context, txHash common.TxID, constAccessor constants.ConstantValues) error {
...
func (s *SlasherV113) checkSignerAndSlash(ctx cosmos.Context, nodes NodeAccounts, blockHeight int64, signers []cosmos.AccAddress, constAccessor constants.ConstantValues) {
	for _, na := range nodes {
		...
		if !found {
			lackOfObservationPenalty, err := s.keeper.GetMimir(ctx, constants.LackOfObservationPenalty.String())
			if lackOfObservationPenalty < 0 || err != nil {
...
func (s *SlasherV113) LackSigning(ctx cosmos.Context, mgr Manager) error {
	var resultErr error
	...
	for i, tx := range txs.TxArray {
		...
		if tx.OutHash.IsEmpty() {
			...
			if vault.Status == InactiveVault {
				ctx.Logger().Info("cannot reassign outbound from inactive vault", "hash", tx.InHash)
				continue
			}

			var nas NodeAccounts
			nas, err = s.keeper.ListActiveValidators(ctx)
			if err != nil {
...
func (s *SlasherV113) IncSlashPoints(ctx cosmos.Context, point int64, addresses ...cosmos.AccAddress) {
...
func (s *SlasherV113) DecSlashPoints(ctx cosmos.Context, point int64, addresses ...cosmos.AccAddress) {
	for _, addr := range addresses {
...
func (s *SlasherV113) SlashVaultToLP(ctx cosmos.Context, vaultPK common.PubKey, coins common.Coins, mgr Manager, subsidize bool) error {
	if coins.IsEmpty() {
		return nil
	}
...
func (s *SlasherV113) SlashNodeAccountLP(ctx cosmos.Context, na NodeAccount, slash cosmos.Uint) (cosmos.Uint, []types.PoolAmt, error) {
	if slash.IsZero() {
		return cosmos.ZeroUint(), nil, nil
	}

	for _, genesis := range GenesisNodes {
		add, err := common.NewAddress(genesis, s.keeper.GetVersion())
		if err != nil {
...
func (s *SlasherV113) needsNewVault(ctx cosmos.Context, mgr Manager, nas int, signingTransPeriod, startHeight int64, inhash common.TxID, pk common.PubKey) bool {
