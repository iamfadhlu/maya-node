...
type ValidatorMgrV108TestSuite struct{}

var _ = Suite(&ValidatorMgrV108TestSuite{})

func (vts *ValidatorMgrV108TestSuite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (vts *ValidatorMgrV108TestSuite) TestSetupValidatorNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	mgr := NewDummyMgr()
	networkMgr := newValidatorMgrV108(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	networkMgr1 := newValidatorMgrV108(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	networkMgr2 := newValidatorMgrV108(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV108TestSuite) TestRagnarokForChaosnet(c *C) {
	ctx, mgr := setupManagerForTest(c)
	networkMgr := newValidatorMgrV108(mgr.Keeper(), mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	for i := 0; i < 12; i++ {
		...
		SetupLiquidityBondForTestV105(c, ctx, mgr.Keeper(), common.BNBAsset, node.BondAddress, node, cosmos.NewUint(uint64(i+1)*common.One))
...
func (vts *ValidatorMgrV108TestSuite) TestLowerVersion(c *C) {
	ctx, mgr := setupManagerForTest(c)
	...
	genesisAdd, err := common.NewAddress(GenesisNodes[0], common.LatestVersion)
	c.Assert(err, IsNil)
...
func (vts *ValidatorMgrV108TestSuite) TestBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (vts *ValidatorMgrV108TestSuite) TestFindBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *ValidatorMgrV108TestSuite) TestFindLowBondActor(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(1000)
	...
	bp.Providers = append(bp.Providers, NewBondProvider(acc))
	bp.Providers[0].Bonded = true
	SetupLiquidityBondForTestV105(c, ctx, mgr.Keeper(), common.BNBAsset, na.BondAddress, na, cosmos.NewUint(10))
	c.Assert(mgr.Keeper().SetBondProviders(ctx, bp), IsNil)
	...
	bp.Providers = append(bp.Providers, NewBondProvider(acc))
	bp.Providers[0].Bonded = true
	SetupLiquidityBondForTestV105(c, ctx, mgr.Keeper(), common.BNBAsset, na2.BondAddress, na2, na2Bond)
	c.Assert(mgr.Keeper().SetBondProviders(ctx, bp), IsNil)
	...
	bp.Providers = append(bp.Providers, NewBondProvider(acc))
	bp.Providers[0].Bonded = true
	SetupLiquidityBondForTestV105(c, ctx, mgr.Keeper(), common.BNBAsset, na3.BondAddress, na3, na3Bond)
	c.Assert(mgr.Keeper().SetBondProviders(ctx, bp), IsNil)
...
func (vts *ValidatorMgrV108TestSuite) TestGetChangedNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := GetCurrentVersion()
	...
	bp.Providers = append(bp.Providers, NewBondProvider(acc))
	bp.Providers[0].Bonded = true
	SetupLiquidityBondForTestV105(c, ctx, k, common.BNBAsset, activeNode.BondAddress, activeNode, activeNodeBond)
...
func (vts *ValidatorMgrV108TestSuite) TestSplitNext(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgr()
	networkMgr := newValidatorMgrV108(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(networkMgr, NotNil)
	...
	for i := 0; i < 90; i++ {
		...
		bp.Providers = append(bp.Providers, NewBondProvider(acc))
		bp.Providers[0].Bonded = true
		SetupLiquidityBondForTestV105(c, ctx, k, common.BNBAsset, na.BondAddress, na, naBond)
		c.Assert(k.SetBondProviders(ctx, bp), IsNil)
...
func (vts *ValidatorMgrV108TestSuite) TestFindCounToRemove(c *C) {
	// remove one
...
func (vts *ValidatorMgrV108TestSuite) TestFindMaxAbleToLeave(c *C) {
...
func (vts *ValidatorMgrV108TestSuite) TestFindNextVaultNodeAccounts(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgrWithKeeper(k)
	networkMgr := newValidatorMgrV108(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(networkMgr, NotNil)
...
func (vts *ValidatorMgrV108TestSuite) TestFindNextVaultNodeAccountsMax(c *C) {
	...
	for i := 0; i < 12; i++ {
		...
		bp.Providers = append(bp.Providers, NewBondProvider(acc))
		bp.Providers[0].Bonded = true
		SetupLiquidityBondForTestV105(c, ctx, mgr.Keeper(), common.BNBAsset, na.BondAddress, na, cosmos.NewUint(100*common.One))
		c.Assert(mgr.Keeper().SetBondProviders(ctx, bp), IsNil)
		if i < 3 {
			na.LeaveScore = 1024
		}
...
func (vts *ValidatorMgrV108TestSuite) TestEquitableBondReward(c *C) {
	ctx, k := setupKeeperForTest(c)
	...
	for i, node := range nodes {
		...
		SetupLiquidityBondForTestV105(c, ctx, k, common.BTCAsset, node.BondAddress, node, cosmos.NewUint(1*common.One))

		extraBP := GetRandomBaseAddress()
		extraBPAcc, _ := extraBP.AccAddress()
		bondAddressAcc, _ := node.BondAddress.AccAddress()
		SetupLiquidityBondForTestV105(c, ctx, k, common.BTCAsset, extraBP, node, cosmos.NewUint((uint64(i)+1)*common.One))
		bp := NewBondProviders(node.NodeAddress)
...
func (vts *ValidatorMgrV108TestSuite) TestActiveNodeRequestToLeaveShouldBeStandby(c *C) {
	var err error
	ctx, mgr := setupManagerForTest(c)
	...
	for i := 1; i <= bonderCount; i++ {
		...
		SetupLiquidityBondForTestV105(c, ctx, mgr.Keeper(), common.BNBAsset, na.BondAddress, na, naBond)
		c.Assert(mgr.Keeper().SetNodeAccount(ctx, na), IsNil)
