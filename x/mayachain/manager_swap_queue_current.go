...
const PreferredAssetSwapMemoPrefix = "MAYA-PREFERRED-ASSET"
	...
	for i := int64(0); i < vm.getTodoNum(int64(len(swaps)), minSwapsPerBlock, maxSwapsPerBlock); i++ {
		...
		if handleErr != nil {
			...
			if pick.msg.IsStreaming() {
				...
				// Should not refund a failed preferred asset swap, the CACAO is still in the AffiliateCollector module
...
func (vm *SwapQueueVCUR) cleanupFailedPreferredAssetSwap(ctx cosmos.Context, mgr Manager, memo string, cacaoAmt cosmos.Uint) error {
	ctx.Logger().Info("preferred asset swap failed, send cacao back to affiliate collector", "runeAmt", cacaoAmt.String(), "memo", memo)
	// get the preferred asset swap's mayaname
	name, ok := strings.CutPrefix(memo, fmt.Sprintf("%s-", PreferredAssetSwapMemoPrefix))
	if !ok {
		return fmt.Errorf("failed to get mayaname from memo: %s", memo)
	}
	if tn, err := vm.k.GetMAYAName(ctx, name); err == nil {
		affCol, errAffCol := mgr.Keeper().GetAffiliateCollector(ctx, tn.Owner)
		if errAffCol != nil {
			return fmt.Errorf("failed to get affiliate collector record: %w", errAffCol)
		} else {
			affCol.CacaoAmount = affCol.CacaoAmount.Add(cacaoAmt)
			mgr.Keeper().SetAffiliateCollector(ctx, affCol)
		}
	} else {
		return fmt.Errorf("failed to get mayaname: %w", err)
	}

	// send cacao back to affiliate collector
	if err := mgr.Keeper().SendFromModuleToModule(ctx, AsgardName, AffiliateCollectorName, common.NewCoins(common.NewCoin(common.BaseAsset(), cacaoAmt))); err != nil {
		return fmt.Errorf("failed to send cacao back to affiliate collector: %w", err)
	}
