...
type validatorMgrV114TestSuite struct{}

var _ = Suite(&validatorMgrV114TestSuite{})

func (vts *validatorMgrV114TestSuite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (vts *validatorMgrV114TestSuite) TestSetupValidatorNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	mgr := NewDummyMgr()
	networkMgr := newValidatorMgrV114(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	networkMgr1 := newValidatorMgrV114(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	networkMgr2 := newValidatorMgrV114(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *validatorMgrV114TestSuite) TestRagnarokForChaosnet(c *C) {
	ctx, mgr := setupManagerForTest(c)
	networkMgr := newValidatorMgrV114(mgr.Keeper(), mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *validatorMgrV114TestSuite) TestLowerVersion(c *C) {
	ctx, mgr := setupManagerForTest(c)
	...
	genesisAdd, err := common.NewAddress(GenesisNodes[0], GetCurrentVersion())
	c.Assert(err, IsNil)
...
func (vts *validatorMgrV114TestSuite) TestBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (vts *validatorMgrV114TestSuite) TestFindBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *validatorMgrV114TestSuite) TestFindLowBondActor(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *validatorMgrV114TestSuite) TestGetChangedNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := GetCurrentVersion()
...
func (vts *validatorMgrV114TestSuite) TestSplitNext(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgr()
	networkMgr := newValidatorMgrV114(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(networkMgr, NotNil)
...
func (vts *validatorMgrV114TestSuite) TestFindCounToRemove(c *C) {
	// remove one
...
func (vts *validatorMgrV114TestSuite) TestFindMaxAbleToLeave(c *C) {
...
func (vts *validatorMgrV114TestSuite) TestFindNextVaultNodeAccounts(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgrWithKeeper(k)
	networkMgr := newValidatorMgrV114(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(networkMgr, NotNil)
...
func (vts *validatorMgrV114TestSuite) TestFindNextVaultNodeAccountsMax(c *C) {
...
func (vts *validatorMgrV114TestSuite) TestEquitableBondReward(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *validatorMgrV114TestSuite) TestActiveNodeRequestToLeaveShouldBeStandby(c *C) {
	var err error
	ctx, mgr := setupManagerForTest(c)
