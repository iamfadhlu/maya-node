...
func (vm *ValidatorMgrVCUR) BeginBlock(ctx cosmos.Context, mgr Manager, existingValidators []string) error {
	...
	for _, chain := range activeChains {
		if isChainHalted(ctx, mgr, chain) {
...
func (vm *ValidatorMgrVCUR) churn(ctx cosmos.Context) error {
	constAccessor := vm.k.GetConstants()
	...
	err := vm.markBadActor(ctx, minSlashPointsForBadValidator, redline)
	if err != nil {
		return err
	}
...
func (vm *ValidatorMgrVCUR) payNodeAccountBondAward(ctx cosmos.Context, lastChurnHeight int64, na NodeAccount, nodeReward, nodeBond sdk.Uint, bpBonds []sdk.Uint, mgr Manager) error {
...
func (vm *ValidatorMgrVCUR) getPendingTxOut(ctx cosmos.Context, constAccessor constants.ConstantValues) (int64, error) {
...
func (vm *ValidatorMgrVCUR) ragnarokBondReward(ctx cosmos.Context, mgr Manager) error {
	var resultErr error
...
func (vm *ValidatorMgrVCUR) setupValidatorNodes(ctx cosmos.Context, height int64, constAccessor constants.ConstantValues) error {
	if height != genesisBlockHeight {
...
func (vm *ValidatorMgrVCUR) markBadActor(ctx cosmos.Context, minSlashPointsForBadValidator, redline int64) error {
...
func (vm *ValidatorMgrVCUR) markLowVersionValidators(ctx cosmos.Context, constAccessor constants.ConstantValues) error {
...
func (vm *ValidatorMgrVCUR) markReadyActors(ctx cosmos.Context, constAccessor constants.ConstantValues) error {
...
func (vm *ValidatorMgrVCUR) NodeAccountPreflightCheck(ctx cosmos.Context, na NodeAccount, constAccessor constants.ConstantValues) (NodeStatus, error) {
	...
	minBond := vm.k.GetConfigInt64(ctx, constants.MinimumBondInCacao)
...
func (vm *ValidatorMgrVCUR) nextVaultNodeAccounts(ctx cosmos.Context, targetCount int, constAccessor constants.ConstantValues) (NodeAccounts, bool, error) {
