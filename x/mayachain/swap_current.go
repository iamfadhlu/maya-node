...
func (s *SwapperVCUR) validateMessage(tx common.Tx, target common.Asset, destination common.Address) error {
	...
	if tx.Coins[0].Asset.IsTradeAsset() && !target.IsTradeAsset() && !target.IsBase() {
		return errors.New("swaps from trade asset to L1 incur slip, use trade-")
	}
	if target.IsTradeAsset() && !tx.Coins[0].Asset.IsTradeAsset() && !tx.Coins[0].Asset.IsBase() {
...
func (s *SwapperVCUR) swapOne(ctx cosmos.Context,
	...
	tradeAccountsWithdrawEnabled := mgr.Keeper().GetConfigInt64(ctx, constants.TradeAccountsWithdrawEnabled)
	...
	if source.IsTradeAsset() {
		...
		if tradeAccountsWithdrawEnabled <= 0 {
			return cosmos.ZeroUint(), evt, fmt.Errorf("trade accounts withdrawals are disabled")
		}
