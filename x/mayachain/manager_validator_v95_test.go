...
func (vts *ValidatorMgrV95TestSuite) TestWeightedBondReward(c *C) {
	...
	c.Assert(err, IsNil)
	pool.LPUnits = cosmos.NewUint(7)
	pool.BalanceCacao = cosmos.NewUint(7 * common.One)
	...
	network.BondRewardRune = cosmos.NewUint(7 * common.One)
	c.Assert(mgr.Keeper().SetNetwork(ctx, network), IsNil)

	setCalcNodeLiquidityBond(c, ctx, k, na1, 3)
	setCalcNodeLiquidityBond(c, ctx, k, na2, 3)
	setCalcNodeLiquidityBond(c, ctx, k, na3, 1)
	...
	// The Reward hard cap in the test environment is 3 * common.One, both na1 and na2 should have the same reward
	...
	// na3.Reward is below the hard cap, it should have a smaller reward accordingly
	c.Check(na3.Reward.Uint64(), Equals, uint64(1*common.One), Commentf("expected %d, got %d", 1*common.One, na3.Reward.Uint64()))
}
