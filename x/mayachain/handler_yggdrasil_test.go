...
func newYggdrasilHandlerTestHelper(c *C) yggdrasilHandlerTestHelper {
	...
	nodeBond := cosmos.NewUint(100 * common.One)
	c.Assert(keeper.SetNodeAccount(ctx, nodeAccount), IsNil)
	SetupLiquidityBondForTest(c, ctx, keeper, common.BTCAsset, nodeAccount.BondAddress, nodeAccount, nodeBond)
	...
	calcNodeBond, err := keeper.CalcNodeLiquidityBond(ctx, nodeAccount)
	c.Assert(err, IsNil)
	c.Assert(calcNodeBond.Equal(nodeBond.MulUint64(2)), Equals, true, Commentf("%d", calcNodeBond))

	constAccessor := constants.GetConstantValues(version)

	mgr := NewDummyMgrWithKeeper(keeper)
	mgr.validatorMgr = newValidatorMgrVCUR(k, mgr.NetworkMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	yggdrasilVault.Coins = append(yggdrasilVault.Coins, common.NewCoin(common.BTCAsset, cosmos.NewUint(common.One*100)))
...
func (s *HandlerYggdrasilSuite) TestYggdrasilHandler(c *C) {
	testCases := []struct {
		...
		},
		{
			...
			messageCreator: func(helper yggdrasilHandlerTestHelper) cosmos.Msg {
				fromAddr, _ := helper.yggVault.PubKey.GetAddress(common.BTCChain)
				coins := common.Coins{
			...
			expectedResult: nil,
			validator: func(helper yggdrasilHandlerTestHelper, msg cosmos.Msg, result *cosmos.Result, c *C) {
				expectedBond := cosmos.NewUint(19895469641)
				na, err := helper.keeper.GetNodeAccountByPubKey(helper.ctx, helper.yggVault.PubKey)
				c.Assert(err, IsNil)
				calcNodeBond, err := helper.keeper.CalcNodeLiquidityBond(helper.ctx, na)
				c.Assert(err, IsNil)
		...
		},
		{
			...
			messageCreator: func(helper yggdrasilHandlerTestHelper) cosmos.Msg {
				...
				coins := common.Coins{
					...
					common.NewCoin(common.BaseAsset(), cosmos.NewUint(common.One)),
				}
				tx := common.Tx{
					ID:          GetRandomTxHash(),
