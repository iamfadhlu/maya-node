...
func (k *TestSwapKeeper) GetPool(ctx cosmos.Context, asset common.Asset) (mayachaintypes.Pool, error) {
	if asset.Equals(common.Asset{Chain: common.BNBChain, Symbol: "NOTEXIST", Ticker: "NOTEXIST"}) {
		return mayachaintypes.Pool{}, nil
	}
	if asset.Equals(common.DASHAsset) {
		return mayachaintypes.Pool{
...
func (k *TestSwapKeeper) SetPool(ctx cosmos.Context, ps mayachaintypes.Pool) error { return nil }

func (k *TestSwapKeeper) GetLiquidityProvider(ctx cosmos.Context, asset common.Asset, addr common.Address) (mayachaintypes.LiquidityProvider, error) {
	if asset.Equals(common.Asset{Chain: common.BNBChain, Symbol: "NOTEXISTSTICKER", Ticker: "NOTEXISTSTICKER"}) {
		return mayachaintypes.LiquidityProvider{}, errors.New("you asked for it")
	}
...
func (k *TestSwapKeeper) SetLiquidityProvider(ctx cosmos.Context, ps mayachaintypes.LiquidityProvider) {
}
...
func (k *TestSwapKeeper) GetMimir(ctx cosmos.Context, key string) (int64, error) {
	if key == constants.SlipFeeAddedBasisPoints.String() {
		return k.addedBps, nil
	}
	return 0, nil
}

func (k *TestSwapKeeper) GetConfigInt64(ctx cosmos.Context, key constants.ConstantName) int64 {
	return k.addedBps
}
...
func (s *SwapVCURSuite) TestSwap(c *C) {
	...
	inputs := []struct {
		...
		},
		{
			...
			requestTxHash: "hash",
			source:        common.DASHAsset.GetSyntheticAsset(),
			target:        common.BaseAsset(),
			amount:        cosmos.NewUint(5 * common.One),
			requester:     GetRandomBaseAddress(),
			destination:   GetRandomBaseAddress(),
		...
		},
		{
			...
			requester:     GetRandomBaseAddress(),
			destination:   GetRandomBNBAddress(),
			returnAmount:  cosmos.NewUint(453038549),
			tradeTarget:   cosmos.NewUint(453038549),
			expectedErr:   nil,
			events:        1,
			addedBps:      10,
		},
		{
			name:          "double-swap-with-added-bps",
			requestTxHash: "hash",
			source:        common.Asset{Chain: common.BTCChain, Ticker: "BTC", Symbol: "BTC"},
			target:        common.BNBAsset,
			amount:        cosmos.NewUint(5 * common.One),
			requester:     GetRandomBaseAddress(),
			destination:   GetRandomBNBAddress(),
			returnAmount:  cosmos.NewUint(414186411),
			tradeTarget:   cosmos.NewUint(414186411),
			expectedErr:   nil,
			events:        2,
			addedBps:      10,
		},
	...
	for _, item := range inputs {
		...
		if item.addedBps != 0 {
			poolStorage.addedBps = item.addedBps
		} else {
			poolStorage.addedBps = 0
		}
...
func (s *SwapVCURSuite) TestSynthSwap_RuneSynthRune(c *C) {
	...
	// swap synth --> rune
	{
		...
		amount, _, err := newSwapperVCUR().Swap(ctx, mgr.Keeper(), tx, common.BaseAsset(), addr, cosmos.ZeroUint(), "", "", nil, StreamingSwap{}, cosmos.NewUint(1000_000), 20_000, mgr)
		c.Assert(err, IsNil)
		c.Check(amount.Uint64(), Equals, swapResult.Uint64(),
			Commentf("Actual: %d Exp: %d", amount.Uint64(), swapResult.Uint64()))
		pool, err = mgr.Keeper().GetPool(ctx, common.BNBAsset)
