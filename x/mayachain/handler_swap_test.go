...
func (s *HandlerSwapSuite) TestProcessPreferredAssetSwap(c *C) {
	...
	msg := NewMsgSwap(tx, common.BaseAsset(), GetRandomBaseAddress(), cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(), "", "", nil, MarketOrder, 0, 0, baseAccAddr)
	err := handler.processPreferredAssetSwap(ctx, *msg)
	c.Assert(err, NotNil)

	// no affiliate collector found for the signer
	tx.Coins = common.NewCoins(common.NewCoin(common.BaseNative, cosmos.NewUint(100)))
	msg = NewMsgSwap(tx, common.BaseAsset(), GetRandomBaseAddress(), cosmos.ZeroUint(), common.NoAddress, cosmos.ZeroUint(), "", "", nil, MarketOrder, 0, 0, baseAccAddr)
	err = handler.processPreferredAssetSwap(ctx, *msg)
	c.Assert(err, NotNil)
	...
	c.Assert(err, IsNil)
	affCol.CacaoAmount = cosmos.NewUint(100)
