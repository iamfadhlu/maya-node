...
func (s TxOutStoreV119Suite) TestEndBlock(c *C) {
	...
	txOutStore := newTxOutStorageV118(w.keeper, w.mgr.GetConstants(), w.mgr.EventMgr(), w.mgr.GasMgr())
...
func (s TxOutStoreV119Suite) TestAddOutTxItem(c *C) {
	...
	testAndCheckModuleBalancesV119(c, w.ctx, w.keeper,
		func() {
			item = TxOutItem{
...
type ModuleBalancesV119 struct {
	Asgard  int64
	Bond    int64
	Reserve int64
	Module  int64
...
func getModuleBalancesV119(c *C, ctx cosmos.Context, k keeper.Keeper) ModuleBalancesV119 {
	return ModuleBalancesV119{
...
func testAndCheckModuleBalancesV119(c *C, ctx cosmos.Context, k keeper.Keeper, runTest func(), expDeltas ModuleBalancesV119) {
	before := getModuleBalancesV119(c, ctx, k)
	runTest()
	after := getModuleBalancesV119(c, ctx, k)
...
type TestCalcKeeperV119 struct {
	keeper.KVStoreDummy
	value map[int64]cosmos.Uint
	mimir map[string]int64
}

func (k *TestCalcKeeperV119) GetPool(ctx cosmos.Context, asset common.Asset) (types.Pool, error) {
	pool := NewPool()
	pool.Asset = asset
...
func (k *TestCalcKeeperV119) GetMimir(ctx cosmos.Context, key string) (int64, error) {
	return k.mimir[key], nil
}

func (k *TestCalcKeeperV119) GetTxOutValue(ctx cosmos.Context, height int64) (cosmos.Uint, error) {
...
func (s TxOutStoreV119Suite) TestcalcTxOutHeight(c *C) {
	...
	txout := TxOutStorageV118{keeper: keeper}
...
func (s TxOutStoreV119Suite) TestAddOutTxItem_SecurityVersusOutboundNumber(c *C) {
	...
	pool.BalanceCacao = cosmos.NewUint(248680012786574)
	err = w.keeper.SetPool(w.ctx, pool)
	c.Assert(err, IsNil)
	///
	...
	pool.BalanceCacao = cosmos.NewUint(46962864904253)
	err = w.keeper.SetPool(w.ctx, pool)
	...
	pool.BalanceCacao = cosmos.NewUint(837898672769246)
	err = w.keeper.SetPool(w.ctx, pool)
	c.Assert(err, NotNil) // Unlike common.BNBAsset (why?), this requires setting the Asset.
	...
	pool.BalanceCacao = cosmos.NewUint(612691971161372)
	err = w.keeper.SetPool(w.ctx, pool)
	c.Assert(err, NotNil) // Unlike common.BNBAsset (why?), this requires setting the Asset.
