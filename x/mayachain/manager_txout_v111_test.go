...
type TxOutStoreV111Suite struct{}

var _ = Suite(&TxOutStoreV111Suite{})

func (s TxOutStoreV111Suite) TestAddGasFees(c *C) {
...
func (s TxOutStoreV111Suite) TestEndBlock(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
	txOutStore := newTxOutStorageV111(w.keeper, w.mgr.GetConstants(), w.mgr.EventMgr(), w.mgr.GasMgr())
...
func (s TxOutStoreV111Suite) TestAddOutTxItem(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
	...
	randomAddr := GetRandomDASHAddress()
	item = TxOutItem{
		Chain:     common.DASHChain,
		ToAddress: randomAddr,
		InHash:    inTxID,
		Coin:      common.NewCoin(common.DASHAsset, cosmos.NewUint(20*common.One)),
		MaxGas: common.Gas{
			common.NewCoin(common.DASHAsset, cosmos.NewUint(10000)),
		},
	}
	txOutStore.ClearOutboundItems(w.ctx)
	...
	msgs, err = txOutStore.GetOutboundItems(w.ctx)
	c.Assert(err, IsNil)
	c.Assert(msgs[0].ToAddress.String(), Equals, randomAddr.String())
...
func (s TxOutStoreV111Suite) TestAddOutTxItem_OutboundHeightDoesNotGetOverride(c *C) {
	SetupConfigForTest()
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
...
func (s TxOutStoreV111Suite) TestAddOutTxItemNotEnoughForFee(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
		common.NewCoin(common.BaseAsset(), cosmos.NewUint(10000*common.One)),
...
func (s TxOutStoreV111Suite) TestAddOutTxItemDeductMaxGasFromYggdrasil(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
		common.NewCoin(common.BaseAsset(), cosmos.NewUint(10000*common.One)),
...
type TestCalcKeeperV111 struct {
	keeper.KVStoreDummy
	value map[int64]cosmos.Uint
	mimir map[string]int64
}

func (k *TestCalcKeeperV111) GetPool(ctx cosmos.Context, asset common.Asset) (types.Pool, error) {
	pool := NewPool()
	pool.Asset = asset
...
func (k *TestCalcKeeperV111) GetMimir(ctx cosmos.Context, key string) (int64, error) {
	return k.mimir[key], nil
}

func (k *TestCalcKeeperV111) GetTxOutValue(ctx cosmos.Context, height int64) (cosmos.Uint, error) {
...
func (s TxOutStoreV111Suite) TestcalcTxOutHeight(c *C) {
	keeper := &TestCalcKeeperV111{
	...
	txout := TxOutStorageV111{keeper: keeper}
...
func (s TxOutStoreV111Suite) TestAddOutTxItem_MultipleOutboundWillBeScheduledAtTheSameBlockHeight(c *C) {
	SetupConfigForTest()
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
...
func (s TxOutStoreV111Suite) TestAddOutTxItemInteractionWithPool(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
...
func (s TxOutStoreV111Suite) TestAddOutTxItemSendingFromRetiredVault(c *C) {
	SetupConfigForTest()
	w := getHandlerTestWrapper(c, 1, true, true)
