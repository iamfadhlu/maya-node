...
func getInternalHandlerMapping(mgr Manager) map[string]MsgHandler {
	...
	case version.GTE(semver.MustParse("1.123.0")): // trade-accounts
		return getInternalHandlerMappingV123(mgr)
	case version.GTE(semver.MustParse("1.118.0")):
		return getInternalHandlerMappingV118(mgr)
	default:
		return getInternalHandlerMappingV1(mgr)
	}
}

func getInternalHandlerMappingV123(mgr Manager) map[string]MsgHandler {
	// New arch handlers
	m := make(map[string]MsgHandler)
...
func getMsgManageMAYANameFromMemo(memo ManageMAYANameMemo, tx ObservedTx, signer cosmos.AccAddress) (cosmos.Msg, error) {
	...
	return NewMsgManageMAYAName(memo.Name, memo.Chain, memo.Address, tx.Tx.Coins[0], memo.Expire, memo.PreferredAsset, memo.AffiliateBps, memo.SubaffiliateBps, memo.SubaffiliateName, memo.Owner, signer), nil
}
...
func processOneTxIn(ctx cosmos.Context, version semver.Version, keeper keeper.Keeper, tx ObservedTx, signer cosmos.AccAddress) (cosmos.Msg, error) {
	switch {
	case version.GTE(semver.MustParse("1.123.0")): // trade-accounts
		return processOneTxInV123(ctx, keeper, tx, signer)
	case version.GTE(semver.MustParse("1.118.0")):
		return processOneTxInV118(ctx, keeper, tx, signer)
...
func processOneTxInV123(ctx cosmos.Context, keeper keeper.Keeper, tx ObservedTx, signer cosmos.AccAddress) (cosmos.Msg, error) {
	...
	case *MsgTradeAccountWithdrawal:
		return newMsg, m.ValidateBasicVersioned(version)
	}
...
func fuzzyAssetMatch(ctx cosmos.Context, keeper keeper.Keeper, asset common.Asset) common.Asset {
	...
	switch {
	case version.GTE(semver.MustParse("1.123.0")): // trade-accounts
		return fuzzyAssetMatchV123(ctx, keeper, asset)
...
func fuzzyAssetMatchV123(ctx cosmos.Context, keeper keeper.Keeper, origAsset common.Asset) common.Asset {
...
func externalAssetMatch(version semver.Version, chain common.Chain, hint string) string {
	...
	case version.GTE(semver.MustParse("1.111.0")):
		return externalAssetMatchV111(version, chain, hint)
...
func externalAssetMatchV111(version semver.Version, chain common.Chain, hint string) string {
	if len(hint) == 0 {
		return hint
	}
	...
	} else if chain == common.XRDChain {
		for _, token := range tokenlist.GetRadixTokenList(version) {
			tokenAddresses = append(tokenAddresses, token.Address)
		}
	}
	uniqueMatchedToken := findUniqueItemBySuffixIgnoreCase(tokenAddresses, hint)
	...
	if len(uniqueMatchedToken) == 0 {
		return hint
	} else {
		return uniqueMatchedToken
	}
}

// Returns a unique item from a list matched by suffix.
// Returns an empty string is there aren't any matches or if there's more than 1 match.
func findUniqueItemBySuffixIgnoreCase(items []string, suffix string) string {
	suffixLowerCase := strings.ToLower(suffix)
	matches := []string{}
	for _, item := range items {
		if strings.HasSuffix(strings.ToLower(item), suffixLowerCase) {
