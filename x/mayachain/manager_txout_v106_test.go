...
type TxOutStoreSuiteV106 struct{}

var _ = Suite(&TxOutStoreSuiteV106{})

func (s TxOutStoreSuiteV106) TestAddGasFees(c *C) {
	ctx, mgr := setupManagerForTest(c)
	tx := GetRandomObservedTx()

	version := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(version)
	mgr.gasMgr = newGasMgrV106(constAccessor, mgr.Keeper())
	err := addGasFees(ctx, mgr, tx)
	c.Assert(err, IsNil)
	c.Assert(mgr.GasMgr().GetGas(), HasLen, 1)
}

func (s TxOutStoreSuiteV106) TestEndBlock(c *C) {
...
func (s TxOutStoreSuiteV106) TestAddOutTxItem(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
	...
	ygg.AddFunds(
		common.Coins{
			...
			common.NewCoin(common.DASHAsset, cosmos.NewUint(40*common.One)),
		},
	)
	...
	txOutStore := newTxOutStorageV106(w.keeper, w.mgr.GetConstants(), w.mgr.EventMgr(), w.mgr.GasMgr())
	...
	testAndCheckModuleBalancesV106(c, w.ctx, w.keeper,
		func() {
			item = TxOutItem{
...
func (s TxOutStoreSuiteV106) TestAddOutTxItem_OutboundHeightDoesNotGetOverride(c *C) {
	SetupConfigForTest()
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
...
func (s TxOutStoreSuiteV106) TestAddOutTxItemNotEnoughForFee(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
		common.NewCoin(common.BaseAsset(), cosmos.NewUint(10000*common.One)),
...
type ModuleBalancesV106 struct {
	Asgard  int64
	Bond    int64
	Reserve int64
	Module  int64
...
func getModuleBalancesV106(c *C, ctx cosmos.Context, k keeper.Keeper) ModuleBalancesV106 {
	return ModuleBalancesV106{
...
func testAndCheckModuleBalancesV106(c *C, ctx cosmos.Context, k keeper.Keeper, runTest func(), expDeltas ModuleBalancesV106) {
	before := getModuleBalancesV106(c, ctx, k)
	runTest()
	after := getModuleBalancesV106(c, ctx, k)
...
func (s TxOutStoreSuiteV106) TestAddOutTxItemDeductMaxGasFromYggdrasil(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
		common.NewCoin(common.BaseAsset(), cosmos.NewUint(10000*common.One)),
...
type TestCalcKeeperV106 struct {
	keeper.KVStoreDummy
	value map[int64]cosmos.Uint
	mimir map[string]int64
}

func (k *TestCalcKeeperV106) GetPool(ctx cosmos.Context, asset common.Asset) (types.Pool, error) {
	pool := NewPool()
	pool.Asset = asset
...
func (k *TestCalcKeeperV106) GetMimir(ctx cosmos.Context, key string) (int64, error) {
	return k.mimir[key], nil
}

func (k *TestCalcKeeperV106) GetTxOutValue(ctx cosmos.Context, height int64) (cosmos.Uint, error) {
...
func (s TxOutStoreSuiteV106) TestcalcTxOutHeight(c *C) {
	keeper := &TestCalcKeeperV106{
...
func (s TxOutStoreSuiteV106) TestAddOutTxItem_MultipleOutboundWillBeScheduledAtTheSameBlockHeight(c *C) {
	SetupConfigForTest()
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
...
func (s TxOutStoreSuiteV106) TestAddOutTxItemInteractionWithPool(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
...
func (s TxOutStoreSuiteV106) TestAddOutTxItemSendingFromRetiredVault(c *C) {
	SetupConfigForTest()
	w := getHandlerTestWrapper(c, 1, true, true)
