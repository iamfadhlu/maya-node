...
// newGasMgrV112 create a new instance of GasMgrV112
...
func (gm *GasMgrV112) GetFee(ctx cosmos.Context, chain common.Chain, asset common.Asset) cosmos.Uint {
	...
	getConfigInt64 := func(ctx cosmos.Context, c constants.ConstantName) int64 {
		val, err2 := gm.keeper.GetMimir(ctx, c.String())
		if val < 0 || err2 != nil {
			val = gm.constantsAccessor.GetInt64Value(c)
		}
		return val
	}
	targetOutboundFeeSurplus := getConfigInt64(ctx, constants.TargetOutboundFeeSurplusRune)
	maxMultiplierBasisPoints := getConfigInt64(ctx, constants.MaxOutboundFeeMultiplierBasisPoints)
	minMultiplierBasisPoints := getConfigInt64(ctx, constants.MinOutboundFeeMultiplierBasisPoints)
...
func (gm *GasMgrV112) GetNetworkFee(ctx cosmos.Context, chain common.Chain) (NetworkFee, error) {
	outboundTxFee := gm.keeper.GetConfigInt64(ctx, constants.OutboundTransactionFee)
