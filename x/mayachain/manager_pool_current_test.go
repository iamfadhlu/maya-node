...
func (s *PoolMgrV112Suite) TestCommitPendingLiquidity(c *C) {
	var err error
	...
	pool := NewPool()
	pool.Asset = asset
	pool.BalanceCacao = cosmos.NewUint(100_000)
	pool.BalanceAsset = cosmos.NewUint(100_000)
	pool.PendingInboundCacao = cosmos.OneUint()
	...
	lp1 := LiquidityProvider{
		...
		PendingCacao:  cosmos.NewUint(501),
	...
	c.Assert(err, IsNil)
	c.Check(pool.BalanceCacao.Uint64(), Equals, uint64(100_501), Commentf("%d", pool.BalanceCacao.Uint64()))
	...
	c.Check(lp.PendingCacao.Uint64(), Equals, uint64(0), Commentf("%d", lp.PendingCacao.Uint64()))
