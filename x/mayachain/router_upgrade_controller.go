...
func (r *RouterUpgradeController) getChainOldAndNewRouters(chain common.Chain) (string, string, error) {
	...
	case common.ARBChain:
		return arbOldRouter, arbNewRouter, nil
	case common.XRDChain:
		return xrdOldRouter, xrdNewRouter, nil
	default:
...
func (r *RouterUpgradeController) getRouterChains(version semver.Version) ([]common.Chain, error) {
	...
	case version.GTE(semver.MustParse("1.111.0")):
		return []common.Chain{common.ETHChain, common.AVAXChain, common.ARBChain, common.XRDChain}, nil
	case version.GTE(semver.MustParse("1.109.0")):
		return []common.Chain{common.ETHChain, common.AVAXChain, common.ARBChain}, nil
...
func (r *RouterUpgradeController) upgradeContract(ctx cosmos.Context, version semver.Version) error {
	switch {
	case version.GTE(semver.MustParse("1.109.0")):
		return r.upgradeContractV109(ctx, version)
	case version.GTE(semver.MustParse("1.94.0")):
...
func (r *RouterUpgradeController) upgradeContractV109(ctx cosmos.Context, version semver.Version) error {
	...
	for _, chain := range chains {
		...
		newRouterAddr, err := common.NewAddress(newRouter, version)
		if err != nil {
...
func (r *RouterUpgradeController) Process(ctx cosmos.Context) {
	...
	if version.LT(semver.MustParse("1.109.0")) {
