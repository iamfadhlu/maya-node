...
func newTssPoolHandler(cliCtx client.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Convert []Blame to single Blame for pre-v124 compatibility
		var blame types.Blame
		if len(req.Blame) > 0 {
			blame = req.Blame[0]
		}
		msg, err := types.NewMsgTssPool(req.InputPubKeys, req.PoolPubKey, req.KeysharesBackup, req.KeygenType, req.Height, blame, req.Chains, addr, req.KeygenTime)
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusBadRequest, err.Error())
			return
		}
