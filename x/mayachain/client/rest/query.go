...
func getHandlerWrapper(q query.Query, storeName string, cliCtx client.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		} else if latest != nil && latest.Block != nil {
			w.Header().Set("X-Mayachain-Height", fmt.Sprintf("%d", latest.Block.Height))
			w.Header().Set("X-Mayachain-Time", latest.Block.Time.Format(time.RFC3339))
		}
