...
func GetCmdSetAztecAddress() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Println(args[0])
			aztecAddress, err := common.NewAddress(args[0], common.LatestVersion)
			if err != nil {
...
func GetCmdObserveTxIns() *cobra.Command {
	...
	cmd.Flags().String("mayanode-api", "", "mayanode api endpoint")
...
func observeTxs(outbound bool) func(cmd *cobra.Command, args []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		mayaNodeAPI, err := cmd.Flags().GetString("mayanode-api")
		if err != nil {
			return err
		}
		...
		// if txids is set, the mayanode api endpoint must be set
		if txids != "" && mayaNodeAPI == "" {
			return fmt.Errorf("--txids requires --mayanode-api")
		}
		...
		if txids != "" {
			...
			for _, txid := range txidList {
				observation, err := findLackingObservation(txid, nodeAddress, mayaNodeAPI, outbound)
				if err != nil {
		...
		if len(observations) == 0 {
			// The detailed messages have already been printed by findLackingObservation
			// Just provide a summary
			fmt.Println("\nNo new observations to broadcast")
			return nil
		}
		...
		// create the message based on the command type
		var msg cosmos.Msg
		if outbound {
...
func findLackingObservation(txid, address, mayanodeAPI string, outbound bool) (*types.ObservedTx, error) {
	// get tx details from mayanode API using explicit in/out voter endpoints
	endpoint := "in"
	if outbound {
		endpoint = "out"
	}
	url := fmt.Sprintf("%s/mayachain/tx/%s/%s", mayanodeAPI, endpoint, txid)
	...
	if len(txDetails.Txs) == 0 {
		fmt.Printf("[%s] no %s observations exist yet for this transaction\n", txid, endpoint)
		return nil, nil
	}
	...
	for _, tx := range txDetails.Txs {
		...
		if signed {
			fmt.Printf("[%s] %s already observed tx with %d others\n", txid, shortAddress, len(tx.Signers))
			continue
		}
	...
	if observation != nil {
		...
		fmt.Printf("[%s] %s has already signed all existing observations\n", txid, shortAddress)
	}
...
func extractOpenAPIObservedTx(otx openapi.ObservedTx) (*types.ObservedTx, error) {
	...
	tx.Tx.FromAddress, err = common.NewAddress(*otx.Tx.FromAddress, common.LatestVersion)
	if err != nil {
		return nil, fmt.Errorf("failed to parse from address: %w", err)
	}

	tx.Tx.ToAddress, err = common.NewAddress(*otx.Tx.ToAddress, common.LatestVersion)
	if err != nil {
