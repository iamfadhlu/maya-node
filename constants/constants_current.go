...
// NewConstantValueVCUR get new instance of ConstantValueCUR
func NewConstantValueVCUR() *ConstantVals {
	return &ConstantVals{
		int64values: map[ConstantName]int64{
			...
			MaxAffiliateFeeBasisPoints:          500,                 // Maximum affiliate fee basis points for swap and add liquidity
			...
			MAYANameGracePeriodBlocks:           432000,              // Number of blocks for which to lock THORName registration to the original owner post expiration (30 days with 6 second block time)
			MultipleAffiliatesMaxCount:          5,                   // Maximum number of peer affiliates (in memo a/b/c/d:10/15/10/20)
			EVMDisableContractWhitelist:         0,                   // enable/disable contract whitelist
			AffiliateFeeTickGranularity:         100,                 // defines the granularity for affiliate fee calculations in basis points fraction; 100 means 1/100th of a basis point (bps) used for higher precision in fee calculations
			CACAOPoolEnabled:                    0,                   // enable/disable CACAO Pool
			CACAOPoolRewardsEnabled:             1,                   // enable/disable CACAO Pool rewards
			CACAOPoolDepositMaturityBlocks:      14400,               // blocks from last deposit to allow withdraw - one day
			ChurnMigrateRounds:                  5,                   // Number of rounds to migrate vaults during churn
			TradeAccountsEnabled:                0,                   // enable/disable trade account
			TradeAccountsDepositEnabled:         0,                   // enable/disable trade account deposits
			TradeAccountsWithdrawEnabled:        0,                   // enable/disable trade account withdrawals
		},
