...
func (s *AddressSuite) TestAddress(c *C) {
	...
	c.Check(addr.IsChain(ZECChain, maxVer), Equals, false)
	c.Check(addr.IsChain(KUJIChain, maxVer), Equals, false)
	c.Check(addr.GetNetwork(maxVer, BASEChain), Equals, MainNet)
	...
	c.Check(addr.IsChain(THORChain, maxVer), Equals, false)
	c.Check(addr.IsChain(BASEChain, maxVer), Equals, false)
	c.Check(addr.IsChain(DASHChain, maxVer), Equals, true)
	c.Check(addr.IsChain(XRDChain, maxVer), Equals, false)
	c.Check(addr.IsChain(ZECChain, maxVer), Equals, false)
	c.Check(addr.IsChain(KUJIChain, maxVer), Equals, false)
	c.Check(addr.GetNetwork(maxVer, BTCChain), Equals, MainNet)
	...
	// addr, err = NewAddress("bc1ppgj0l0jng3s2t54h5tckjxjdv4zkzpcwfdd4u7vhp63dgml96hds9eyvn6", LatestVersion)
	...
	addr, err = NewAddress("XnaAwMFdebTu1W51q59zf2dzAT3MQRoWSB", LatestVersion)
	c.Check(err, IsNil)
	c.Check(addr.IsChain(BTCChain, maxVer), Equals, false)
	c.Check(addr.IsChain(ETHChain, maxVer), Equals, false)
	c.Check(addr.IsChain(BNBChain, maxVer), Equals, false)
	...
	c.Check(addr.IsChain(ZECChain, maxVer), Equals, false)
	c.Check(addr.IsChain(KUJIChain, maxVer), Equals, false)
	c.Check(addr.GetNetwork(maxVer, DASHChain), Equals, MainNet)

	addr, err = NewAddress("yYCmxJL5697yMEzZPvUPh44LSjXitgWyBc", LatestVersion)
	c.Check(err, IsNil)
	c.Check(addr.IsChain(BTCChain, maxVer), Equals, false)
	c.Check(addr.IsChain(ETHChain, maxVer), Equals, false)
	c.Check(addr.IsChain(BNBChain, maxVer), Equals, false)
	...
	c.Check(addr.IsChain(ZECChain, maxVer), Equals, false)
	c.Check(addr.IsChain(KUJIChain, maxVer), Equals, false)
	c.Check(addr.GetNetwork(maxVer, DASHChain), Equals, TestNet)

	// kuji tests
	addr, err = NewAddress("kujira1jq7n8p38p33cazfd5dqrlq2s580fd9cy5m4j3y", LatestVersion)
	c.Assert(err, IsNil)
	c.Check(addr.IsChain(THORChain, maxVer), Equals, false)
	c.Check(addr.IsChain(BASEChain, maxVer), Equals, false)
	c.Check(addr.IsChain(BNBChain, maxVer), Equals, false)
	...
	c.Check(addr.IsChain(DASHChain, maxVer), Equals, false)
	c.Check(addr.IsChain(XRDChain, maxVer), Equals, false)
	c.Check(addr.IsChain(KUJIChain, maxVer), Equals, true)
	c.Check(addr.IsChain(ZECChain, maxVer), Equals, false)
	c.Check(addr.GetNetwork(maxVer, KUJIChain), Equals, CurrentChainNetwork)

	// radix tests
	validRadixMainnetAddresses := []string{
		"account_loc169nfm5ejkxymcq8ggf894k4n7y6r7344793493uxth5pg7al5nzxt8",
		"loc169nfm5ejkxymcq8ggf894k4n7y6r7344793493uxth5pg7al5nzxt8",
		"169nfm5ejkxymcq8ggf894k4n7y6r7344793493uxth5pg7al5nzxt8",
		"69nfm5ejkxymcq8ggf894k4n7y6r7344793493uxth5pg7al5nzxt8",
	}
	for _, addrStr := range validRadixMainnetAddresses {
		...
		c.Check(addr.GetNetwork(maxVer, XRDChain), Equals, MockNet)
	}
	addr, err = NewAddress("account_loc169nfm5ejkxymcq8ggf894k4n7y6r7344793493uxth5pg7al5nzxt8", LatestVersion)
	c.Assert(err, IsNil)
	c.Check(addr.GetChain(LatestVersion), Equals, XRDChain)
	c.Check(addr.AbbreviatedString(LatestVersion), Equals, "69nfm5ejkxymcq8ggf894k4n7y6r7344793493uxth5pg7al5nzxt8")
	...
	zcashAddresses := []addrTest{
		...
		// TODO: these correct addresses dont pass the bech32m checksum validation, need to investigate
		...
		{"tmGn7qKhnpbnwZPsaPt9XXAQz1Q3HAUCrwh", false},
		// Unified Address
