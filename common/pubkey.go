...
func (pubKey PubKey) EVMPubkeyToAddress() (Address, error) {
	...
	pub, err := btcec.ParsePubKey(pk.Bytes(), btcec.S256())
	if err != nil {
		return NoAddress, err
	}
	str := strings.ToLower(eth.PubkeyToAddress(*pub.ToECDSA()).String())
	return NewAddress(str, LatestVersion)
}
...
func (pubKey PubKey) GetAddress(chain Chain) (Address, error) {
	...
	switch chain {
	case BNBChain, BASEChain, THORChain, KUJIChain:
		pk, err := cosmos.GetPubKeyFromBech32(cosmos.Bech32PubKeyTypeAccPub, string(pubKey))
		if err != nil {
			return NoAddress, err
		}
	...
	case ZECChain:
		var net *zec.NetworkParams
		switch chainNetwork {
		case MockNet, TestNet:
			net = &zec.RegtestNetParams
		case MainNet, StageNet:
			net = &zec.MainNetParams
		}
		addr, err := zec.NewAddressWitnessPubKeyHash(pubKey.String(), net)
		if err != nil {
			return NoAddress, fmt.Errorf("fail to get zec address from public key: %w", err)
		}
		addressString = addr
	case DASHChain:
		pk, err := cosmos.GetPubKeyFromBech32(cosmos.Bech32PubKeyTypeAccPub, string(pubKey))
	...
	case XRDChain:
		...
		var radixPk ret.PublicKey
		switch SigningAlgo(pk.Type()) {
		case SigningAlgoSecp256k1:
			radixPk = ret.PublicKeySecp256k1{
				Value: pk.Bytes(),
			}
		case SigningAlgoEd25519:
			radixPk = ret.PublicKeyEd25519{
				Value: pk.Bytes(),
			}
		default:
			// See chain.GetSigningAlgo
			return NoAddress, fmt.Errorf("%v key type not supported, only secp256k1 and ed25519 are supported for now", pk.Type())
		}

		addr, err := ret.DeriveVirtualAccountAddressFromPublicKey(radixPk, networkId)
		if err != nil {
			return NoAddress, fmt.Errorf("fail to encode the address, err: %w", err)
		}
		addressString = addr.AsStr()
	default:
	...
	address, err := NewAddress(addressString, LatestVersion)
	if err != nil {
		return address, err
	}
