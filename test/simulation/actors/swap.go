...
func (a *SwapActor) acquireUser(config *OpConfig) OpResult {
	...
	pool, err := mayanode.GetPool(a.from)
	if err != nil {
		return OpResult{
			Continue: false,
		}
	...
	for _, user := range config.UserAccounts {
		...
		a.fromAddress, err = user.PubKey(a.to.Chain).GetAddress(a.from.Chain)
		if err != nil {
...
func (a *SwapActor) getQuote(config *OpConfig) OpResult {
	quote, err := mayanode.GetSwapQuote(a.from, a.to, a.swapAmount)
	if err != nil {
	...
	if quote.ExpectedAmountOut == "" {
		a.Log().Error().Str("amount", a.swapAmount.String()).Msg("failed to get swap quote")
		return OpResult{
			Continue: false,
		}
	}
...
func (a *SwapActor) sendSwap(config *OpConfig) OpResult {
	// get inbound address
	inboundAddr, _, err := mayanode.GetInboundAddress(a.from.Chain)
	if err != nil {
...
func (a *SwapActor) verifyOutbound(config *OpConfig) OpResult {
	// get swap stages
	stages, err := mayanode.GetTxStages(a.swapTxID)
	if err != nil {
