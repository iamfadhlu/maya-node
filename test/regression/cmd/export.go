...
func export(out io.Writer, path string, routine int, failExportInvariants bool) error {
	...
	cmd := exec.Command("mayanode", "export")
	...
	mayachain, _ := appState["mayachain"].(map[string]any)
	delete(mayachain, "store_version")

	// ignore node account version for comparison
	nodeAccounts, _ := mayachain["node_accounts"].([]interface{})
...
func checkExportInvariants(out io.Writer, genesis map[string]any) error {
	...
	// encode mayachain state to json for custom unmarshal
	buf := bytes.NewBuffer(nil)
	enc := json.NewEncoder(buf)
	enc.SetIndent("", "  ")
	err := enc.Encode(appState["mayachain"])
	if err != nil {
		log.Fatal().Err(err).Msg("failed to encode genesis state")
	}

	// unmarshal json to genesis state
	genesisState := &mayachain.GenesisState{}
	...
	// TODO: After fixing AsgardInvariant (x/mayachain/keeper/v1/invariants.go:13) we can uncomment this part
