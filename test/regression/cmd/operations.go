...
func (op *OpCreateBlocks) Execute(out io.Writer, path string, _, routine int, p *os.Process, logs chan string) error {
	...
	for i := 0; i < op.Count; i++ {
		...
		url := fmt.Sprintf("http://localhost:%d/mayachain/block?height=%d", 1317+routine, height)
...
func checkInvariants(routine int) error {
	...
	for _, inv := range invariants {
		...
		go func(inv string) {
			...
			endpoint := fmt.Sprintf("%s/mayachain/invariant/%s", api, inv)
...
func (op *OpTxTssPool) Execute(out io.Writer, _ string, _, routine int, _ *os.Process, logs chan string) error {
	msg, err := types.NewMsgTssPoolV2(op.PubKeys, op.PoolPubKey, op.KeysharesBackup, op.KeygenType, op.Height, op.Blame, op.Chains, op.Signer, op.KeygenTime, op.PoolPubKeyEddsa, op.KeysharesBackupEddsa)
	if err != nil {
		return err
	}
	return sendMsg(out, routine, msg, op.Signer, op.Sequence, op.Gas, op, logs)
...
func sendMsg(out io.Writer, routine int, msg sdk.Msg, signer sdk.AccAddress, seq, gas *int64, op any, logs chan string) error {
	...
	if gas != nil {
		...
		txf = txf.WithGas(uint64(defaultGas))
	}
