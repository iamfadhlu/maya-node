...
type PipelineSuite struct {
	vault1 common.PubKey
	vault2 common.PubKey
	tosis  []TxOutStoreItem
}

var _ = Suite(&PipelineSuite{})

func (s *PipelineSuite) SetUpSuite(c *C) {
	mayachain.SetupConfigForTest()

	s.vault1 = ttypes.GetRandomPubKey()
	s.vault2 = ttypes.GetRandomPubKey()

	s.tosis = []TxOutStoreItem{
		{
			TxOutItem: types.TxOutItem{
				Chain:       common.BTCChain,
				ToAddress:   ttypes.GetRandomBTCAddress(),
				VaultPubKey: s.vault1,
			},
		},
		{
			TxOutItem: types.TxOutItem{
...
func (s *PipelineSuite) TestPipelineSequential(c *C) {
	...
	mockSigner := &mockPipelineSigner{
		storageListItems: append([]TxOutStoreItem{}, s.tosis...),
	}
	...
	c.Assert(mockSigner.processed[0].TxOutItem.Equals(s.tosis[0].TxOutItem), Equals, true)
	...
	c.Assert(mockSigner.processed[1].TxOutItem.Equals(s.tosis[1].TxOutItem), Equals, true)
	c.Assert(mockSigner.processed[2].TxOutItem.Equals(s.tosis[2].TxOutItem), Equals, true)
	c.Assert(mockSigner.processed[3].TxOutItem.Equals(s.tosis[3].TxOutItem), Equals, true)
}
...
func (s *PipelineSuite) TestPipelineConcurrent(c *C) {
	...
	c.Assert(mockSigner.storageListItems[0].TxOutItem.Equals(s.tosis[2].TxOutItem), Equals, true)
...
func (s *PipelineSuite) TestPipelineConcurrentLimited(c *C) {
	...
	c.Assert(mockSigner.storageListItems[0].TxOutItem.Equals(s.tosis[2].TxOutItem), Equals, true)
	c.Assert(mockSigner.storageListItems[1].TxOutItem.Equals(s.tosis[3].TxOutItem), Equals, true)
	c.Assert(mockSigner.storageListItems[2].TxOutItem.Equals(s.tosis[4].TxOutItem), Equals, true)
	...
	c.Assert(mockSigner.storageListItems[0].TxOutItem.Equals(s.tosis[4].TxOutItem), Equals, true)
...
func (s *PipelineSuite) testPipelineRetry(c *C, round7, broadcast bool) {
	...
	retryTosis := append([]TxOutStoreItem{}, s.tosis...)
