...
func NewSigner(cfg config.BifrostSignerConfiguration,
	...
	storage, err := NewSignerStore(cfg.SignerDbPath, cfg.LevelDB, mayachainBridge.GetConfig().SignerPasswd)
	if err != nil {
	...
	for _, item := range na.GetSignerMembership() {
		pubkeyMgr.AddPubKey(item, true, common.SigningAlgoSecp256k1)
	}
	if na.PubKeySet.Secp256k1.IsEmpty() {
...
func (s *Signer) processTransactions() {
	signerConcurrency, err := s.mayachainBridge.GetMimir(constants.SignerConcurrency.String())
	if err != nil {
	...
	s.pipeline.SpawnSignings(s, s.mayachainBridge)
}
...
func (s *Signer) processKeygenBlock(keygenBlock ttypes.KeygenBlock) {
	...
	for _, keygenReq := range keygenBlock.Keygens {
		...
		if err := s.sendKeygenToMayachain(keygenBlock.Height, pubKey.Secp256k1, blame, keygenReq.GetMembers(), keygenReq.Type, keygenTime, pubKey.Ed25519); err != nil {
...
func (s *Signer) sendKeygenToMayachain(height int64, poolPk common.PubKey, blame []ttypes.Blame, input common.PubKeys, keygenType ttypes.KeygenType, keygenTime int64, poolPubKeyEddsa common.PubKey) error {
	...
	version, err := s.mayachainBridge.GetMayachainVersion()
	if s.cfg.BackupKeyshares && err == nil {
		// TODO: Remove if/else condition after v1.124.0
	...
	keygenMsg, err := s.mayachainBridge.GetKeygenStdTx(poolPk, keyshares, blame, input, keygenType, chains, height, keygenTime, poolPubKeyEddsa, keysharesEddsa)
	if err != nil {
...
func (s *Signer) signAndBroadcast(item TxOutStoreItem) ([]byte, *types.TxInItem, error) {
	...
	if item.Round7Retry {
		mimirKey := constants.MaxOutboundAttempts.String()
		var maxOutboundAttemptsMimir int64
		maxOutboundAttemptsMimir, err = s.mayachainBridge.GetMimir(mimirKey)
		if err != nil {
		...
		vault, err = s.mayachainBridge.GetVault(item.TxOutItem.VaultPubKey.String())
		if err != nil {
			log.Err(err).
	...
	for _, txArray := range txOut.TxArray {
		if txArray.TxOutItem(height).Equals(tx) && !txArray.OutHash.IsEmpty() {
