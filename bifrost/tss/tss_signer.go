...
func (s *KeySign) makeSignature(msg tx.StdSignMsg, poolPubKey string) (sig tx.StdSignature, err error) {
	...
	signPack, _, err := s.RemoteSign(hashedMsg, common.SigningAlgoSecp256k1, poolPubKey)
	if err != nil {
...
func (s *KeySign) getVersion() semver.Version {
	...
	if !s.currentVersion.Equals(semver.Version{}) && requestTime.Sub(s.lastCheck).Seconds() < constants.MayachainBlockTime.Seconds() {
		return s.currentVersion
	}
	version, err := s.bridge.GetMayachainVersion()
	if err != nil {
