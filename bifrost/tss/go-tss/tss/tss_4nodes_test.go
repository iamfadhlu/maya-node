...
var (
	testPubKeys = []string{
		...
		"mayapub1addwnpepqtdklw8tf3anjz7nn5fly3uvq2e67w2apn560s4smmrt9e3x52nt2kudy4z",
	}
...
func (s *FourNodeTestSuite) SetUpTest(c *C) {
	...
	s.bootstrapPeer = "/ip4/127.0.0.1/tcp/16666/p2p/16Uiu2HAm2FzqoUdS6Y9Esg2EaGcAG5rVe1r6BFNnmmQr2H3bqafa"
...
func (s *FourNodeTestSuite) Test4NodesTss(c *C) {
	algos := []mcommon.SigningAlgo{mcommon.SigningAlgoEd25519, mcommon.SigningAlgoSecp256k1}
...
// generate a new key
func (s *FourNodeTestSuite) doTestKeygenAndKeySign(c *C, version string, algo mcommon.SigningAlgo) {
	wg := sync.WaitGroup{}
	lock := &sync.Mutex{}
...
func (s *FourNodeTestSuite) doTestFailJoinParty(c *C, version string, algo mcommon.SigningAlgo) {
	...
	for _, item := range keygenResult {
		...
		expectedFailNode := "mayapub1addwnpepqfjcw5l4ay5t00c32mmlky7qrppepxzdlkcwfs2fd5u73qrwna0vzmc8mup"
...
func (s *FourNodeTestSuite) doTestBlame(c *C, version string, algo mcommon.SigningAlgo) {
	wg := sync.WaitGroup{}
	lock := &sync.Mutex{}
	keygenResult := make(map[int]keygen.Response)
...
func (s *FourNodeTestSuite) getTssServer(c *C, index int, conf common.TssConfig, bootstrap string) *TssServer {
	...
	instance, err := NewTss(peerIDs, s.ports[index], priKey, priKey, "Asgard", baseHome, conf, s.preParams[index], "")
	c.Assert(err, IsNil)
	return instance
}
