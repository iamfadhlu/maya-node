...
type Communication struct {
	...
	subscribers      map[messages.MAYAChainTSSMessageType]*MessageIDSubscriber
...
func NewCommunication(rendezvous string, bootstrapPeers []maddr.Multiaddr, port int, externalIP string) (*Communication, error) {
	...
	return &Communication{
		...
		subscribers:      make(map[messages.MAYAChainTSSMessageType]*MessageIDSubscriber),
		subscriberLocker: &sync.Mutex{},
		streamCount:      0,
...
func (c *Communication) SetSubscribe(topic messages.MAYAChainTSSMessageType, msgID string, channel chan *Message) {
...
func (c *Communication) getSubscriber(topic messages.MAYAChainTSSMessageType, msgID string) chan *Message {
	c.subscriberLocker.Lock()
	defer c.subscriberLocker.Unlock()
	messageIDSubscribers, ok := c.subscribers[topic]
	if !ok {
...
func (c *Communication) CancelSubscribe(topic messages.MAYAChainTSSMessageType, msgID string) {
	c.subscriberLocker.Lock()
	defer c.subscriberLocker.Unlock()
