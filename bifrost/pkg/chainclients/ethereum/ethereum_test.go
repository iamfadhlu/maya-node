...
import (
	...
	"gitlab.com/mayachain/mayanode/bifrost/pkg/chainclients/shared"
...
func (s *EthereumSuite) SetUpTest(c *C) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		case mayaclient.ChainVersionEndpoint:
		...
		case "/mayachain/mimir/key/MaxUTXOsToSpend":
			...
			c.Assert(err, IsNil)
		case "/mayachain/mimir/key/ConfMultiplierBasisPoints-ETH":
			_, err := rw.Write([]byte(fmt.Sprintf("%d", shared.ETHChainConfig.ConfMultiplierBasisPoints)))
			c.Assert(err, IsNil)
		case "/mayachain/mimir/key/MaxConfirmations-ETH":
			_, err := rw.Write([]byte(fmt.Sprintf("%d", shared.ETHChainConfig.MaxConfirmations)))
			c.Assert(err, IsNil)
		default:
...
func (s *EthereumSuite) TestGetAsgardAddresses(c *C) {
	...
	c.Assert(pubkeyMgr.Start(), IsNil)
	defer func() { c.Assert(pubkeyMgr.Stop(), IsNil) }()
	addresses, err := e.getAsgardAddresses()
	c.Assert(err, IsNil)
...
func (s *EthereumSuite) TestGetConfirmationCount(c *C) {
	...
	asgardAddresses, err := e.getAsgardAddresses()
	c.Assert(err, IsNil)
	...
	// test conf multiplier basis points
	c.Assert(e.GetConfirmationCount(stypes.TxIn{
		Chain: common.ETHChain,
		TxArray: []stypes.TxInItem{
			{
				...
				To:          asgardAddresses[0].String(),
				Coins: common.Coins{
					common.NewCoin(common.ETHAsset, cosmos.NewUint(3e9)),
				},
				ObservedVaultPubKey: pubkey,
			},
		},
		...
		TxArray: []stypes.TxInItem{
			{
				...
				To:          asgardAddresses[0].String(),
				Coins: common.Coins{
					common.NewCoin(common.ETHAsset, cosmos.NewUint(8e9)),
				},
				ObservedVaultPubKey: pubkey,
			},
		},
