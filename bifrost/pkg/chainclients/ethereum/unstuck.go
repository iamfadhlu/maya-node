...
func (c *Client) unstuck() {
	...
	defer c.logger.Info().Msg("stop ETH chain unstuck process")
	defer c.wg.Done()
	for {
		select {
		case <-c.stopchan:
...
func (c *Client) unstuckAction() {
	...
	if err != nil {
		c.logger.Err(err).Msg("fail to get MAYAChain block height")
		return
	}
	...
	if err != nil {
		c.logger.Err(err).Msg("failed to get MAYAChain constants")
		return
	}
	...
	for _, item := range signedTxItems {
		...
		if item.Height > height {
			clog.Warn().Msg("signed outbound height greater than current mayachain height")
			continue
		}
