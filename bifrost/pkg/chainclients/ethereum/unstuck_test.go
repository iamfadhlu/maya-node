...
import (
	...
	ttypes "gitlab.com/mayachain/mayanode/bifrost/mayaclient/types"
...
func (s *UnstuckTestSuite) SetUpTest(c *C) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		case mayaclient.MayachainConstants:
...
func (s *UnstuckTestSuite) TestUnstuckProcess(c *C) {
	...
	txID1 := types.GetRandomTxHash().String()
	txID2 := types.GetRandomTxHash().String()
	// add some thing here
	c.Assert(e.ethScanner.blockMetaAccessor.AddSignedTxItem(evmtypes.SignedTxItem{
		Hash:        txID1,
		Height:      1022,
		VaultPubKey: pubkey,
	}), IsNil)
	...
	inHash := types.GetRandomTxHash()
	toi := ttypes.TxOutItem{
		Chain:       common.ETHChain,
		ToAddress:   types.GetRandomETHAddress(),
