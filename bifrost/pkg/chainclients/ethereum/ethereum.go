...
func (c *Client) convertSigningAmount(amt *big.Int, token string) *big.Int {
	// convert 1e8 to 1e18
	amt = common.ConvertMayachainAmountToWei(amt)
	if IsETH(token) {
		return amt
	}
...
func (c *Client) SignTx(tx stypes.TxOutItem, height int64) ([]byte, []byte, *stypes.TxInItem, error) {
	...
	if c.cfg.BlockScanner.FixedGasRate > 0 || gasRate.Cmp(big.NewInt(0)) == 0 {
		...
		gasRate = common.ConvertMayachainAmountToWei(big.NewInt(tx.GasRate))
	} else {
		...
		lowerBound := common.ConvertMayachainAmountToWei(big.NewInt(tx.GasRate))
	...
	c.logger.Info().
		...
		Str("outboundRate", common.ConvertMayachainAmountToWei(big.NewInt(tx.GasRate)).String()).
	...
	if tx.Aggregator != "" {
		...
		if err != nil {
			c.logger.Err(err).
				...
				Msg("fail to get aggregator gas limit, aborting to let mayanode reschedule")
			return nil, nil, nil, nil
		}
		...
		if estimatedGas > gasLimitForAggregator {
			c.logger.Warn().
				...
				Msg("aggregator gas limit exceeded, aborting to let mayanode reschedule")
			return nil, nil, nil, nil
		}
	...
	if c.cfg.BlockScanner.FixedGasRate == 0 {
		...
		// if estimated gas is more than the planned gas, abort and let mayanode reschedule
...
func (c *Client) BroadcastTx(txOutItem stypes.TxOutItem, hexTx []byte) (string, error) {
	...
	if err != nil {
		c.logger.Err(err).Msgf("fail to get current MAYAChain block height")
...
// ConfirmationCountReady check whether the given txIn is ready to be send to MAYAChain
...
// getBlockRequiredConfirmation find out how many confirmation the given txIn need to have before it can be send to MAYAChain
func (c *Client) getBlockRequiredConfirmation(txIn stypes.TxIn, height int64) (int64, error) {
	asgards, err := c.getAsgardAddresses()
	if err != nil {
	...
	totalTxValueInWei := common.ConvertMayachainAmountToWei(totalTxValue.BigInt())
...
func (c *Client) getAsgardAddresses() ([]common.Address, error) {
	...
	newAddresses, err := utxo.GetAsgardAddresses(common.ETHChain, c.bridge)
	if err != nil {
