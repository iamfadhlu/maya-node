...
import (
	...
	. "gitlab.com/mayachain/mayanode/bifrost/pkg/chainclients/shared/types"
...
func NewEVMScanner(cfg config.BifrostBlockScannerConfiguration,
	...
	return &EVMScanner{
		...
		vaultABI:             vaultABI,
		erc20ABI:             erc20ABI,
		eipSigner:            etypes.NewArbitrumSigner(etypes.NewLondonSigner(chainID)),
		pubkeyMgr:            pubkeyMgr,
...
func (e *EVMScanner) FetchTxs(height, chainHeight int64) (stypes.TxIn, error) {
	...
	if e.solvencyReporter != nil {
		if err := e.solvencyReporter(height); err != nil {
			e.logger.Err(err).Msg("failed to report Solvency info to MAYANode")
		}
	}
...
func (e *EVMScanner) getTxIn(block *etypes.Block) (stypes.TxIn, error) {
	...
	for _, batch := range batches {
		...
		err := e.ethClient.Client().BatchCallContext(context.Background(), rpcBatch)
		if err != nil {
...
// updateGasPrice calculates and stores the current gas price to reported to mayanode
...
// reportNetworkFee reports current network fee to mayanode
func (e *EVMScanner) reportNetworkFee(height int64) {
	...
	// gas price to 1e8
	var mcGasPrice *big.Int
	if e.cfg.ChainID.Equals(common.ARBChain) {
		// we use milligwei for arbitrum
		mcGasPrice = new(big.Int).Div(gasPrice, big.NewInt(10_000_000))
	} else {
		mcGasPrice = new(big.Int).Div(gasPrice, big.NewInt(common.One*100))
	}

	// post to mayachain
	e.logger.Info().Uint64("last_gas_price", e.lastReportedGasPrice).Uint64("gas_price", gasPrice.Uint64()).Msg("posting network fee")
	if _, err := e.bridge.PostNetworkFee(height, e.cfg.ChainID, e.cfg.MaxGasLimit, mcGasPrice.Uint64()); err != nil {
		e.logger.Err(err).Msg("failed to post EVM chain single transfer fee to MAYANode")
	} else {
...
func (e *EVMScanner) getTxInFromSmartContract(tx *etypes.Transaction, receipt *etypes.Receipt, maxLogs int64) (*stypes.TxInItem, error) {
	...
	e.logger.Debug().Str("tx hash", txInItem.Tx).Str("gas price", tx.GasPrice().String()).Uint64("gas used", receipt.GasUsed).Uint64("tx status", receipt.Status).Msg("txInItem parsed from smart contract")

	// under no circumstance EVM gas price will be less than 1 Gwei, unless it is in dev environment
	txGasPrice := tx.GasPrice()
	if txGasPrice.Cmp(big.NewInt(tenGwei)) < 0 && !e.cfg.ChainID.Equals(common.ARBChain) {
		txGasPrice = big.NewInt(tenGwei)
	} else if txGasPrice.Cmp(big.NewInt(centiGwei)) < 0 && e.cfg.ChainID.Equals(common.ARBChain) {
		txGasPrice = big.NewInt(centiGwei)
	}
	// TODO: remove version check after v118
	version, err := e.bridge.GetMayachainVersion()
	if err != nil {
		return nil, fmt.Errorf("fail to get mayachain version: %w", err)
	}
...
// transaction is an outbound , and report it back to mayanode, thus the gas fee can be
...
func (e *EVMScanner) getTxInFromFailedTransaction(tx *etypes.Transaction, receipt *etypes.Receipt) *stypes.TxInItem {
	...
	version, err := e.bridge.GetMayachainVersion()
	if err != nil {
		e.logger.Err(err).Msg("fail to get mayachain version")
		return nil
	}
	if version.GTE(semver.MustParse("1.118.0")) {
