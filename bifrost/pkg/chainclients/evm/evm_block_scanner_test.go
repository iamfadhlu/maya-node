...
import (
	...
	mayacommon "gitlab.com/mayachain/mayanode/common"
...
func (s *BlockScannerTestSuite) TestProcessBlock(c *C) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		default:
			...
			if strings.HasPrefix(req.RequestURI, mayaclient.MimirEndpoint+"/key") {
...
func (s *BlockScannerTestSuite) TestProcessReOrg(c *C) {
	...
	rpcClient, err := evm.NewEthRPC(server.URL, time.Second, "ETH") // rpcClient, err := evm.NewEthRPC(ethClient, time.Second, "BSC") in TC test
	c.Assert(err, IsNil)
	cfg := getConfigForTest(server.URL)
	cfg.ChainID = mayacommon.ETHChain // re-org on ETH only // BSC in TC test
...
func (s *BlockScannerTestSuite) TestUpdateGasPrice(c *C) {
	...
	for i := 0; i < 50; i++ {
		bs.updateGasPrice([]*big.Int{
			big.NewInt(4 * TestGasPriceResolution),
			big.NewInt(8 * TestGasPriceResolution),
			big.NewInt(12 * TestGasPriceResolution),
			big.NewInt(16 * TestGasPriceResolution),
		})
	}
	c.Assert(len(bs.gasCache), Equals, 100)
	c.Assert(bs.gasPrice.String(), Equals, big.NewInt(12*TestGasPriceResolution).String())
}
