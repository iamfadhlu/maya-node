...
func (c *EVMClient) buildOutboundTx(txOutItem stypes.TxOutItem, memo mem.Memo, nonce uint64) (*etypes.Transaction, error) {
	...
	if c.cfg.BlockScanner.FixedGasRate > 0 || gasRate.Cmp(big.NewInt(0)) == 0 {
		...
		gasRate = c.evmConvertMayachainAmount(big.NewInt(txOutItem.GasRate))
	} else {
		...
		lowerBound := c.evmConvertMayachainAmount(big.NewInt(txOutItem.GasRate))
		lowerBound.Mul(lowerBound, big.NewInt(2))
		lowerBound.Div(lowerBound, big.NewInt(3))

		factor := int64(common.MayachainAmountToWeiFactor)
		if c.cfg.ChainID.IsARB() {
			factor = common.MayachainAmountToCentiWeiFactor
		}

		// round current rate to avoid consensus trouble, same rounding implied in outbound
		gasRate.Div(gasRate, big.NewInt(factor))
		if gasRate.Cmp(big.NewInt(0)) == 0 { // floor at 1 like in network fee reporting
			gasRate = big.NewInt(1)
		}
		gasRate.Mul(gasRate, big.NewInt(factor))
	...
	c.logger.Info().
		...
		Str("outboundRate", c.evmConvertMayachainAmount(big.NewInt(txOutItem.GasRate)).String()).
	...
	for _, coin := range txOutItem.MaxGas {
		scheduledMaxFee.Add(scheduledMaxFee, common.ConvertMayachainAmountToWei(coin.Amount.BigInt()))
	}
	...
	if txOutItem.Aggregator != "" {
		...
		if err != nil {
			c.logger.Err(err).
				...
				Msg("fail to get aggregator gas limit, aborting to let mayanode reschdule")
			return nil, nil
		}

		// if the estimate gas is over the max, abort and let mayanode reschedule for now
		if estimatedGas > gasLimitForAggregator {
			c.logger.Warn().
				...
				Msg("swap out gas limit exceeded, aborting to let mayanode reschedule")
			return nil, nil
		}
...
func (c *EVMClient) SignTx(tx stypes.TxOutItem, height int64) ([]byte, []byte, *stypes.TxInItem, error) {
	...
	if tx.Checkpoint != nil {
		...
		finalizedNonce, err = c.evmScanner.ethRpc.GetNonceFinalized(fromAddr.String())
		if err != nil {
	...
	// if transaction is nil, abort to allow mayanode reschedule
	if outboundTx == nil {
		return nil, nil, nil, nil
	}
...
func (c *EVMClient) evmConvertMayachainAmount(txOutItemGasRate *big.Int) *big.Int {
	if c.cfg.ChainID.Equals(common.ARBChain) {
		return common.ConvertMayachainAmountToCentiWei(txOutItemGasRate)
	}
	return common.ConvertMayachainAmountToWei(txOutItemGasRate)
}
