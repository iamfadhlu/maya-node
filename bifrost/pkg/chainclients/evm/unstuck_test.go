...
func (s *UnstuckTestSuite) SetUpTest(c *C) {
	...
	c.Assert(err, IsNil)
	s.mayaKeys = mayaclient.NewKeysWithKeybase(kb, cfg.SignerName, cfg.SignerPasswd)

	// get public key
	priv, err := s.mayaKeys.GetPrivateKey()
	c.Assert(err, IsNil)
	...
	s.bridge, err = mayaclient.NewMayachainBridge(cfg, s.m, s.mayaKeys)
	c.Assert(err, IsNil)
}
...
func (s *UnstuckTestSuite) TestUnstuckProcess(c *C) {
	...
	e, err := NewEVMClient(s.mayaKeys, config.BifrostChainConfiguration{
