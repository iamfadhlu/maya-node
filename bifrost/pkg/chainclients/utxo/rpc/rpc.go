...
// SerializableTx interface already defined in interface.go
...
// Ensure Client implements UTXOClient interface
var _ UTXOClient = (*Client)(nil)
...
func (c *Client) ImportAddressRescan(address string, rescan bool) error {
	err := c.Call(nil, "importaddress", address, "", rescan)
	return extractBTCError(err)
}
...
// Call invokes a raw RPC method on the node.
