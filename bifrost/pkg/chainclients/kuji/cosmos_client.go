...
func NewCosmosClient(
	...
	// CHANGEME: each THORNode network (e.g. mainnet, testnet, etc.) may connect to a Kuji chain with a different chain ID
	...
	c.kujiScanner, err = NewKujiBlockScanner(
		c.cfg.BlockScanner,
		c.storage,
		c.mayachainBridge,
		m,
	...
	c.blockScanner, err = blockscanner.NewBlockScanner(c.cfg.BlockScanner, c.storage, m, c.mayachainBridge, c.kujiScanner)
	if err != nil {
...
func (c *KujiClient) GetHeight() (int64, error) {
	return c.kujiScanner.GetHeight()
}
...
func (c *KujiClient) GetAccountByAddress(address string, _ *big.Int) (common.Account, error) {
	...
	var acc *atypes.QueryAccountResponse
	...
	for _, coin := range tx.Coins {
		...
		if strings.HasPrefix(tx.Memo, "YGGDRASIL-:") {
			if coin.Asset == c.cfg.ChainID.GetGasAsset() {
				...
				subtractFee := c.kujiScanner.averageFee().Mul(ctypes.NewUint(3)).Quo(ctypes.NewUint(2))
...
func (c *KujiClient) SignTx(tx stypes.TxOutItem, mayachainHeight int64) (signedTx, _ []byte, _ *stypes.TxInItem, err error) {
	defer func() {
		if err != nil {
	...
	currentHeight, err := c.kujiScanner.GetHeight()
	if err != nil {
	...
	if len(gasCoins) != 1 {
		...
		if strings.HasPrefix(tx.Memo, "YGGDRASIL-:") {
			gasCoins = append(gasCoins, common.NewCoin(
				...
				c.kujiScanner.averageFee().Mul(ctypes.NewUint(3)).Quo(ctypes.NewUint(2)),
			))
		} else {
...
func (c *KujiClient) ReportSolvency(blockHeight int64) error {
	...
	for _, asgard := range asgardVaults {
		...
		if runners.IsVaultSolvent(acct, asgard, c.kujiScanner.lastFee) && c.IsBlockScannerHealthy() {
			continue
		}
		select {
...
func (c *KujiClient) ShouldReportSolvency(height int64) bool {
	...
	// positives.
	return height%10 == 0 && !c.kujiScanner.lastFee.IsZero()
}
