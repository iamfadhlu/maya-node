...
// the MAYAChain Router. This is determined by checking if the tx data is at least 4
// bytes and there is at least one log in the receipt. It is possible for a smart
// contract call to have no logs or no data, but these cannot be MAYAChain deposits, so
// they can be parsed as a normal tx. On the other hand, simple ETH/ERC20 transfer CAN
// have logs & data, but again these will not be MAYAChain deposits or outbounds.
