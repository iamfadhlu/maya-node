...
func NewCosmosClient(
	...
	statusUrl := os.Getenv("THOR_STATUS_URL")
	if statusUrl == "" {
		statusUrl = fmt.Sprintf("%s/status", cfg.RPCHost)
	}
	chainID, err := getThorchainsChainID(statusUrl)
	if err != nil {
		return nil, fmt.Errorf("fail to fetch thorchain chain ID: %w", err)
	}
...
func (c *CosmosClient) GetAccountByAddress(address string, _ *big.Int) (common.Account, error) {
	...
	for _, balance := range balances.Balances {
		...
		coin, err = fromThorchainToMayachain(balance)
		if err != nil {
	...
	for _, coin := range tx.Coins {
		...
		if strings.HasPrefix(tx.Memo, "YGGDRASIL-:") {
			if coin.Asset == c.cfg.ChainID.GetGasAsset() {
				subtractFee := c.cosmosScanner.lastFee
		...
		cosmosCoin, err = fromMayachainToThorchain(coin)
		if err != nil {
...
func (c *CosmosClient) SignTx(tx stypes.TxOutItem, thorchainHeight int64) (signedTx, checkpoint []byte, _ *stypes.TxInItem, err error) {
	...
	// Set THORChains fee to zero
	if !gasCoins[0].Amount.IsZero() {
		gasCoins[0].Amount = cosmos.ZeroUint()
	}

	cCoin, err := fromMayachainToThorchain(gasCoins[0])
	if err != nil {
...
// getThorchainsChainID queries THORChains chain id for stagenet/mainnet, for mocknet returns THORChainMocknetChainID
func getThorchainsChainID(url string) (string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return "", errors.New("Status code: " + resp.Status + " returned")
	}
	if err != nil {
		return "", fmt.Errorf("failed to read response body: %w", err)
	}

	var response ThorchainStatusResponse
