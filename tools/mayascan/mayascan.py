...
API_ENDPOINT = os.getenv("API_ENDPOINT", "https://mayanode.mayachain.info")
RPC_ENDPOINT = os.getenv("RPC_ENDPOINT", "https://tendermint.mayachain.info")
...
def _get(*args, **kwargs) -> requests.Response:
    ...
    kwargs["headers"]["x-client-id"] = "mayascan"
...
def _fetch_block(height: int) -> Optional[Dict[str, Any]]:
    while True:
        if stop_scan:
            return None
        res = _get(f"{API_ENDPOINT}/mayachain/block", params={"height": height})
