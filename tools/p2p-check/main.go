...
func main() {
	...
	url := fmt.Sprintf("%s/mayachain/nodes", getEnvOrDefault("MAYANODE", "https://mayanode.mayachain.info"))
	// nolint
	resp, err := http.Get(url)
	if err != nil {
		fmt.Println("fail to get mayanode status", err.Error())
	} else {
		defer resp.Body.Close()

		if err = json.NewDecoder(resp.Body).Decode(&nodes); err != nil {
			fmt.Println("fail to decode mayanode status", err.Error())
		} else {
	...
	height, err := fetchBlockHeight(getEnvOrDefault("RPC", "https://tendermint.mayachain.info"))
	if err != nil {
		panic(err)
	}
	wg := sync.WaitGroup{}
	...
	for peerID, ipAddr := range nodesByPeerID {
		...
		go func(peerID, ipAddr string) {
			...
			if height > nodeHeight+5 {
				r.description = fmt.Sprintf("mayanode behind the tip %d", height-nodeHeight)
				results[node.NodeAddress] = r
				return
			}
		}(peerID, ipAddr)
	...
	if len(results) > 0 {
		...
		for k, v := range results {
			fmt.Println(k[len(k)-4:], "\t", v.node.BondAddress[len(k)-4:], "\t\t", v.node.Status, "\t", v.description)
		}
...
func fetchBlockHeight(addr string) (int64, error) {
	...
	if resp.StatusCode > 300 {
		...
		return 0, fmt.Errorf("mayanode status bad request: %d", resp.StatusCode)
	}
