...
func (s *HealthServer) p2pStatus(w http.ResponseWriter, _ *http.Request) {
	...
	for _, pi := range peerInfos {
		...
		go func(pi tss.PeerInfo) {
			...
			// check if the node is in mayanode
...
func (s *HealthServer) currentSigning(w http.ResponseWriter, _ *http.Request) {
	...
	mayanode := config.GetBifrost().MayaChain.ChainHost
	url := fmt.Sprintf("http://%s%s", mayanode, mayaclient.AsgardVault)
	resp, err := http.Get(url)
	if err != nil {
		s.logger.Error().Err(err).Msg("fail to get mayanode status")
	} else {
...
func (s *HealthServer) chainScanner(w http.ResponseWriter, _ *http.Request) {
	...
	// Fetch mayachain height
	mayanode := config.GetBifrost().MayaChain.ChainHost
	url := fmt.Sprintf("http://%s/mayachain/lastblock", mayanode)
	resp, err := http.Get(url)
	if err != nil {
		s.logger.Error().Err(err).Msg("fail to get mayanode status")
	} else {
		defer resp.Body.Close()
		var height int64
		height, err = strconv.ParseInt(resp.Header.Get("x-mayachain-height"), 10, 64)
		if err != nil {
			s.logger.Error().Err(err).Msg("fail to parse mayanode height")
		}
		res[common.BASEChain.String()] = ScannerResponse{
			Chain:              common.BASEChain.String(),
			ChainHeight:        height,
			BlockScannerHeight: -1, // TODO: pending for mayachain
			ScannerHeightDiff:  -1,
		}
	}
